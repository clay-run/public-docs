...
**Important:** Always invite users to your workspace before they sign in with SSO. If a user signs in via SSO before receiving their workspace invite, they will be placed into a new standalone personal workspace instead of your enterprise workspace. Every subsequent SSO login will continue to route them to that personal workspace until they accept a workspace invite.

**If a user has already landed in a personal workspace instead of the company workspace:** Send them a workspace invite from `Settings` > `Team` > `+ Invite` — you do not need to remove the user first. The user should click the invite link from their email. They can accept the invite while already signed into Clay: they will be taken to a workspace invite confirmation screen and, after accepting, will have access to the enterprise workspace. Once the user visits the enterprise workspace at least once, subsequent SSO logins will route them there automatically. The personal workspace created by mistake remains as a separate workspace but does not need to be deleted for the recovery to work. If the user did not receive the invite email, ask them to check their spam folder, or resend the invite from `Settings` > `Team`. If the user continues to be routed to the personal workspace on subsequent logins after accepting the invite, contact Clay support via the in-app chat.
