...
4.  Once your IT team confirms the WorkOS setup is complete, notify Clay support. **Note:** Clay's SSO uses SP-initiated login only — IdP-initiated login (clicking the Clay tile directly in your IdP dashboard, such as Okta or Azure AD) is not supported. If you test at this point by clicking the Clay tile in your IdP dashboard, you will see `{"type":"BadRequest","message":"Unable to login","details":null}`. To test SSO, go to the Clay login page, enter your email, and click **Continue** to be redirected to your identity provider.
...
-   Clay's SSO uses **SP-initiated login only**. Users sign in by going to the Clay login page, entering their email, and clicking **Continue** — Clay's domain detection redirects them to their identity provider automatically. **IdP-initiated login is not supported**: clicking a Clay tile directly in your IdP's dashboard (for example, clicking the Clay tile in Okta or Azure AD without first navigating to Clay) will return `{"type":"BadRequest","message":"Unable to login","details":null}`.
