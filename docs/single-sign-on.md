...
4.  Once the WorkOS setup is complete and tested, SSO activates automatically on Clay's side within a few minutes — no additional step from Clay support is required. **Note:** If you test by clicking the Clay tile in your IdP dashboard immediately after completing the WorkOS setup and see `{"type":"BadRequest","message":"Unable to login","details":null}`, wait a few minutes for the automatic activation to complete and try again. If the error persists after 10 minutes, contact Clay support.
