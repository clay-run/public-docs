...
**If a user has already landed in a personal workspace instead of the company workspace:** From your company workspace, go to `Settings` > `Team` > `+ Invite` and send them a workspace invite. Have the user accept the invite from their email — before signing in through the IdP — and they will be directed to the correct workspace on that login. If this approach does not work — for example, the user continues to be routed to the personal workspace after accepting the invite, or they never received the invite email — contact Clay support via the in-app chat to delete the duplicate workspace. Once it has been removed, the workspace admin should remove the user from the enterprise workspace in `Settings` > `Team`, re-invite them using `+ Invite`, and have the user accept the new invite email before signing in through the IdP.
