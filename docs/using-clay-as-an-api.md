...
**Note: Each workspace has a default limit of 5 tables that can be enabled for API sync, and turning off "Enable for API" on a table does not always free up a slot.** The error **"Synced tables limit reached (X/X). Contact us to raise your limit."** appears in **Table Settings → Integrations → Enable for API** when your workspace is at its limit. A table counts toward this limit if the **Enable for API** toggle is currently on, *or* if it has ever completed at least one successful API sync — even if the toggle has since been turned off. Turning off the toggle stops future syncing, but the slot remains consumed as long as that table's synced data exists in Clay's store. Deleted tables also continue to hold a slot until their underlying synced data is cleaned up. To check how many slots your workspace is using, run `clay tables query-usage` from the CLI. To request a higher limit, [contact Clay support](https://www.clay.com/contact-form).
...
**Note: `clay workflows actions search` is now available on the stable channel (open beta).** Submit a natural-language query and get back ranked enrichment actions and workspace functions that match what you want a workflow to do. `clay workflows actions search "<query>"` returns `{ "query": <string>, "candidates": [...], "searchId"?: <string> }`. Each candidate has `type` (`action` or `function`), `candidateId`, `name`, `score`, and optional `description`. Actions additionally include `packageId`, `actionKey`, `displayName`, `provider`, `inputParameters`, `outputParameters`, and optional `creditCost`, `usesPrivateKeyCost`, `requiresApiKey`, `dataStrengths`, and `whyUseful` — `creditCost` and `usesPrivateKeyCost` are catalog base prices, not live quotes; missing prices mean unknown, not free. Functions include `functionId`, `inputSchema`, `outputSchema`, and optional `requiredInputNames`. Pass `--limit <n>` (1–10; default 10) to cap the number of candidates returned. Requires `cli:all` or `terracotta:cli` scope on your API key. Common errors: `not_found` (exit 6) if the workspace's search endpoint has been disabled; `auth_forbidden` (exit 3) if workflow access is denied; `validation_error` (exit 2) if the query is empty or exceeds 2,000 characters, or the limit is outside 1–10.
