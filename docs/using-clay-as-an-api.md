...
**Note: Four `clay audiences` commands let you read Audiences data from the CLI.** All four are available in the stable channel and require Audiences to be enabled for the workspace (available on Growth and Enterprise plans — see [Audiences](https://university.clay.com/docs/audiences)):

-   `clay audiences fields list --entity-type <people|companies|deals>` — list the field definitions for an entity type (id, name, data type, and other metadata).
...
-   `clay audiences records search-ids --entity-type <people|companies|deals> [--audience-id <id> | --filter <file>] [--cursor <cursor>]` — return matching record ids, cursor-paginated at 50 per page. Pass `--audience-id` to scope to a saved audience, `--filter` to apply an ad-hoc filter AST, or omit both to iterate all records of that entity type. Feed the returned ids to `clay audiences records get` in batches of 100 to retrieve field values. **Deals are searchable only as a whole entity type: passing `--audience-id` or a non-empty `--filter` alongside `--entity-type deals` returns `validation_error` (exit 2). To find deal records matching specific criteria, filter companies or people by their deals and then read the deal records via `records get`.**
-   `clay audiences records search-count --entity-type <people|companies|deals> [--audience-id <id> | --filter <file>]` — return the count of records matching a scope without paging through ids. **The same deals restriction applies: `--audience-id` and non-empty `--filter` are not accepted with `--entity-type deals`.**

All four commands return `auth_forbidden` (exit 3) if Audiences is not enabled for the workspace. `records get` additionally returns `rate_limited` (exit 4) if the workspace exceeds its hourly record budget or per-minute request rate — back off for `details.retryAfter` seconds before retrying.
