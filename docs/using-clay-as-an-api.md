...
**Custom integrations via DCR:** Clay's MCP OAuth server supports Dynamic Client Registration (DCR). Once DCR is enabled for your workspace, register a new OAuth client by `POST`ing to `https://api.clay.com/oauth/register` with your desired `redirect_uris` (any HTTPS URI or HTTP loopback `http://localhost:<port>/callback` is accepted); the response returns your `client_id` and `client_secret`. The required scope is `mcp`. Contact [Clay support](https://www.clay.com/contact-form) to request DCR access for your workspace.

**Headless / server-to-server use:** Clay's OAuth server supports only `authorization_code` and `refresh_token` grant types — there is no `client_credentials` grant. For a backend that runs without a human in the loop, run the interactive `authorization_code` + PKCE flow once with a dedicated service user (a Clay workspace member), capture the `refresh_token`, and have your backend use `grant_type=refresh_token` from then on. Register as a confidential client (`token_endpoint_auth_method: "client_secret_basic"`) and use a loopback redirect URI (e.g. `http://localhost:<port>/callback`) for the one-time authorization step. Access is gated by the service user's workspace membership — there is no additional per-client enablement flag beyond DCR access itself.
