...
**Note: `clay schedules` commands are experimental and require `CLAY_CLI_CHANNEL=experimental`.** GTM Agent schedules are recurring prompts that run the GTM Agent on a cadence — each occurrence starts a fresh agent task as the schedule's creator. The four subcommands are `create`, `list`, `update`, and `delete`; there is no `get` subcommand. Each workspace is capped at 20 schedules.

`clay schedules list` returns a paginated list (--limit 1–200, default 50; --cursor for continuation) of the schedules you created. Each schedule object includes `status` and `currentTask`. `status` is the schedule's **lifecycle** — whether it will run again — with three values: `"active"` (running on its cadence), `"paused"` (you paused it, or its cadence stopped parsing), or `"completed"` (retired — no further occurrences, or manually retired while keeping runs readable). `currentTask` is `{ "id": string, "status": "pending"|"queued"|"running"|"waiting_for_user" }` while a run is unsettled, or `null` once every run has settled. The two fields are independent: a paused schedule can still have a run finishing, and an active schedule whose run is blocked on a question reports `status: "active"` with `currentTask.status: "waiting_for_user"`. To find schedules waiting for user input: `clay schedules list | jq '.data[] | select(.currentTask.status == "waiting_for_user")'`. Common errors: `not_found` (exit 6) if the GTM Agent is not enabled for the workspace; `auth_forbidden` (exit 3) if you are not an editor.
