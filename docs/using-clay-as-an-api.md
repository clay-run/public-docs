...
-   `clay audiences records search-ids --entity-type <people|companies> [--audience-id <id> | --filter <file>] [--limit <n>] [--cursor <cursor>]` — return matching record ids, cursor-paginated. `--limit` controls the page size (1–10,000; default 50). Pass `--audience-id` to scope to a saved audience, `--filter` to apply an ad-hoc filter AST, or omit both to iterate all records of that entity type. Feed the returned ids to `clay audiences records get` in batches of 100 to retrieve field values.
