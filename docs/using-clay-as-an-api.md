...
**Note: `clay workspaces` commands let you manage multiple signed-in Clay workspaces.** When you sign into a second workspace with `clay login`, the first stays stored. `clay workspaces list` shows every workspace with a stored credential and which one is currently active. `clay workspaces switch <id>` points all subsequent commands at a workspace you already hold — it confirms the credential against Clay before the switch is kept; pass an `id` from `clay workspaces list`. `clay workspaces current` prints the active workspace's id and name without a network call. When more than one workspace is signed in, `clay logout` removes only the active workspace's credentials and leaves the rest intact, and `clay whoami` also returns a `signedInWorkspaces` array listing every signed-in workspace (id, name, and active status) alongside the workspace it resolves to live.
