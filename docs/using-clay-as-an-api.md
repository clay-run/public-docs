...
**Note: `clay audiences activities get` and `clay audiences activities summary` are experimental CLI commands for reading segment activity data.** Both require Audiences to be enabled for the workspace (available on Growth and Enterprise plans — see [Audiences](https://university.clay.com/docs/audiences)) and the experimental CLI channel. To access them, set `CLAY_CLI_CHANNEL=experimental` before running. In managed agent sessions (Sculptor and GTM), the experimental channel is enabled automatically when the workspace has the experimental channel flag on — contact [Clay support](https://www.clay.com/contact-form) to request access.

-   `clay audiences activities get --segment-id <id> --since <date> [--until <date>] --activity-types <types> [--sources <sources>] [--limit <n>] [--cursor <cursor>]` — return activities for records in the segment, cursor-paginated by activity id. `--since` and `--activity-types` are required on the first page. Pass the returned `cursor` back via `--cursor` to continue pagination; do not combine `--cursor` with `--since`, `--until`, `--activity-types`, or `--sources`. Page size is 1–1000 (default 250). The `--since`/`--until` lookback window cannot exceed 365 days.
-   `clay audiences activities summary --segment-id <id> --since <date> [--until <date>] --activity-types <types> [--sources <sources>]` — return activity counts grouped by type, subtype, and source for the segment. Both `--since` and `--activity-types` are required.

Both commands return `auth_forbidden` (exit 3) if Audiences is not enabled for the workspace, `not_found` (exit 6) if the segment does not exist, and `rate_limited` (exit 4) if the workspace exceeds its activity request rate — back off for `details.retryAfter` seconds before retrying.
