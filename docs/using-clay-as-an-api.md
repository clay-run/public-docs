...
**Note: `clay credits balance` returns the workspace remaining credit balance.** `clay credits balance` outputs `{ "balance": <number>, "actionExecutionBalance": <number|undefined> }` — the current data credit balance and, when present, a separate action-execution credit balance. The command is available on both the stable and experimental channels. Common error: `auth_forbidden` (exit 3) if the key lacks the `cli:all` scope or you lack read access to the workspace.

**Note: `clay workflows analysis` provides read-only access to workflow run execution data.** All five subcommands are available on both the stable and experimental channels and require the `terracotta:cli` scope; run analysis must also be enabled for the workspace (you will see `auth_forbidden` if it is not):

-   `clay workflows analysis traffic <workflowId>` — per-edge transition counts, per-node reach/failure/duration stats, and where runs ended, scoped to the latest workflow version.
-   `clay workflows analysis aggregate [--workflow <workflowId>] --input '<json>'` — aggregates run and step statistics grouped by up to two dimensions (`groupBy` is required; choices include `workflow`, `status`, `trigger`, `snapshot_version`, time buckets, and — when scoped to a single workflow — `node` and `error_message`). Returns counts, failure counts, duration stats (avg and p50/p75/p90/p95/p99), and credits per group.
-   `clay workflows analysis runs [--workflow <workflowId>]` — lists runs with status, timing, credits, and error text, filterable by status, trigger, version, and time range.
-   `clay workflows analysis steps <workflowId>` — samples real step inputs and outputs for a workflow, filterable by node, status, run ids, or time range.
-   `clay workflows analysis search "<query>" [--workflow <workflowId>]` — token search across recorded run execution content (step inputs/outputs, prompts, agent reasoning, tool results), returning matches with snippets and run attribution.

Common error for all five: `auth_forbidden` (exit 3) if run analysis is not enabled for the workspace or the key lacks the `terracotta:cli` scope.
