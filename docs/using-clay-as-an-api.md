...
**Note: `clay workflows nodes create-account-agent` is available on the stable channel for Workflows Open Beta workspaces.** `clay workflows nodes create-account-agent <workflowId> <nodeId>` creates an account agent from the platform template and links it to an agent node that has no linked agent yet; use `--name` to set the agent's display name (defaults to "Account agent"). Currently in beta — available to workspaces in the Workflows Open Beta; contact [Clay support](https://www.clay.com/contact-form) to enable. Returns `{ nodeId, accountAgent: { id } }`. Common errors: `auth_forbidden` (exit 3) if account agents are not enabled for the workspace or the key lacks edit access; `validation_error` (exit 2) if the node already has a linked agent or a precondition is unmet; `not_found` (exit 6) if the workflow or node id does not exist.
