...
**Note: The Clay CLI is not fully read-only.** Table creation and row-data editing are not available programmatically — those operations stay in the Clay UI. The CLI does support writes in other areas: `clay workflows create --name <name>` creates a new empty workflow, `clay workflows runs test <workflowId>` starts a workflow test run, and `clay routines runs start <routineId>` submits an async routine run (a routine must be individually enabled for API & CLI access first — see the not_found error in the routines docs). Functions and workflows can also be triggered via the Public HTTP API Routines endpoint (see section 3). Claygents are nodes inside workflows and run as part of workflow execution — there is no CLI or API command to trigger a Claygent independently; run the workflow that contains it.
