...
**Note: Authentication failures on `api.clay.com/public/v0` have two distinct causes.** (1) **Wrong key type:** The workspace-scoped key (prefixed `clay_scoped_`) is the only type that authenticates successfully to the Public API. The personal API key (prefixed `clay_user_`), found under **Settings → Your profile → API key**, is for Clay-native in-app integrations only — using it with Public API endpoints returns an authentication error. Always use a key from **Settings → Account → API keys**, not your profile page. (2) **Workspace not provisioned:** If you are using a `clay_scoped_` key but still cannot authenticate, your workspace may not have the Public HTTP API enabled. Regenerating the key will not fix this. [Contact Clay support](https://www.clay.com/contact-form) to request workspace enablement.
...
**System requirements:** The Clay CLI runs on **macOS and Linux**. Native Windows is not supported — the CLI launcher exits with an error on Windows. Windows users can run the CLI using **WSL (Windows Subsystem for Linux)**: install WSL via [Microsoft's WSL guide](https://learn.microsoft.com/en-us/windows/wsl/install), then follow the standard Linux installation steps in the [agent-plugins repository](https://github.com/clay-run/agent-plugins). If you run into issues even with WSL, contact [Clay support](https://www.clay.com/contact-form).
...
**Note: For the REST API, `failed` is a per-item status — not a run-level terminal status.** When polling `GET /routines/run/{routine_run_id}/results` for an inline run, the only run-level terminal status is `complete`. Each item within that completed run then has its own `status` field: `"complete"` if that record was enriched successfully, or `"failed"` if that specific record could not be enriched. For batch runs, the run-level terminal statuses are `complete`, `validation_failed`, and `processing_failed`; `in_progress` is non-terminal for both inline and batch runs.

**Note: Completed batch run result files are JSONL with a pre-signed download URL.** When a batch run reaches `complete`, the `result_url` in the response is a pre-signed URL — no `clay-api-key` header is required to download it. The URL expires 15 minutes after it is generated, so download promptly. Each line in the file is a JSON object matching the inline `RunResultItem` structure: `{ "id": string, "status": "complete" | "failed", "result"?: { ... }, "error"?: { "message": string } }`.
...
**Note: `clay tables query-live` is an experimental command available without a paid plan.** It queries a table's live Postgres data directly using ClayQL (no natural-language step), complementing `clay tables query` (which runs against the synced ClickHouse store and requires a paid plan — see the query-sync note below). Results are capped at 100 rows; use `LIMIT n OFFSET m` in your query to page through more. To access it, set `CLAY_CLI_CHANNEL=experimental` before running the command. Available to workspace Admins, Members, and Viewers — not SalesRep-role users.
...
**Note: `POST /public/v0/tables/query` (and `clay tables query`) require two access levels before rows can be returned.** (1) **Workspace plan:** The query-sync feature is not available on the Free plan — Free workspaces receive `403 {"message":"API table sync is not enabled for this workspace."}`. All current paid plans (Trial, Starter, Pro, Growth, Enterprise) include query sync for up to 5 tables by default; if you are on a paid plan but still see this 403, [contact Clay support](https://www.clay.com/contact-form) to have it enabled for your workspace. (2) **Per-table opt-in:** Each table you want to query must have query sync enabled individually. Open the table in Clay, click **Table Settings**, and turn on query sync (you can also enable it via the CLI: `clay tables update <table-id> --query-enabled true`). A table that has not had query sync enabled returns `400` with message: `Tables do not have ClayQL sync enabled: "Table Name" (table-id)`. To raise your workspace's query-synced table limit beyond 5, [contact Clay support](https://www.clay.com/contact-form).

**Note: Deduplicate by `id` when paginating through `/tables/query` scan results.** Scans return rows in least-recently-updated-first order and reflect concurrent writes — a record updated mid-scan may appear again on a later page with fresher data. If you need each record exactly once, deduplicate by the `id` field in each returned row.

**Note: When using `group_by` in a tables query, every `select` column that is not the grouping key must use an aggregate function.** The `group_by` field accepts an array of up to 5 column-name strings. Grouping collapses many rows into one row per distinct group value, so every other column in `select` must resolve to a single value per group — either via `count`, `sum`, `avg`, `min`, or `max`. A plain column that is neither the group key nor an aggregate is ambiguous across the rows in a group — Clay rejects the query with a 400 and the message: `Select field "<field>" must appear in group_by or be aggregated — a grouped query can only project group keys and aggregate functions.` To get a row count per group, use: `"select": [{"field": "Column Name"}, {"fn": "count", "field": "*", "as": "row_count"}], "group_by": ["Column Name"]`. If you want the raw per-row values without a rollup, omit `group_by` entirely — that is why the query works without it. This behavior applies to both the REST API (`POST /public/v0/tables/query`) and the CLI (`clay tables query`).
