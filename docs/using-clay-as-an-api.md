...
**Note:** On SSH sessions, containers, or other headless machines where a local browser cannot open, use `clay login --device` instead of `clay login`. The standard `clay login` flow opens a browser via a local loopback redirect — that redirect fails in headless environments. `clay login --device` prints a verification URL (with your code pre-filled) to the terminal; open the URL in any browser on any device to approve the sign-in, and the CLI completes authentication automatically.
