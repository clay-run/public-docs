...
**Note: `clay tables update --query-enabled true` does not work on function tables.** ClayQL sync cannot be enabled for function tables — the API rejects the request with the error `ClayQL sync cannot be enabled for subroutine tables`. If you need to query function outputs programmatically, route the function's results to a standard enrichment table (using a Send Table Data column inside the function), then enable `--query-enabled true` on that destination table instead. See [Functions](https://university.clay.com/docs/functions) for details on this pattern.
