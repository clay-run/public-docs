...
**Note: `clay tables query` and `POST /public/v0/tables/query` each require an Enterprise plan AND a per-table "Enable for API" toggle.** Two prerequisites must both be met: (1) **Enterprise plan** — non-Enterprise workspaces return `403 "API table sync is not enabled for this workspace."` and the Integrations section (where the per-table toggle lives) does not appear in Table Settings; (2) **per-table "Enable for API" toggle** — even on an Enterprise workspace, each table must be individually enabled: open **Table Settings → Integrations → Enable for API** ("Allow this table to be read via API") and turn it on, then click **Save changes**. The CLI equivalent is `clay tables update <tableId> --query-enabled`. Querying a table that has not been enabled returns `400 "Tables do not have ClayQL sync enabled: <table name> (<tableId>)"`. Enterprise workspaces can have up to 5 tables enabled simultaneously; if you reach the cap, Table Settings shows "Synced tables limit reached" — contact [Clay support](https://www.clay.com/contact-form) to raise the limit.
