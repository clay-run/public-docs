...
**Note: `clay schedules` commands require the GTM Agent feature to be enabled for your workspace (currently in beta — contact [Clay support](https://www.clay.com/contact-form) to request access). `clay schedules update` accepts `"completed"` as a requestable status, and schedule responses include a `hasRun` field.** `clay schedules update <id> --input '{"status":"completed"}'` permanently retires a GTM Agent schedule — use this to stop a schedule that has already run, since `clay schedules delete` is refused once any occurrence has dispatched. Use `"paused"` to stop a schedule temporarily with the option to resume; use `"completed"` to retire it for good. Every schedule response includes `"hasRun": <boolean>` — `true` once an occurrence has been claimed. When `hasRun` is `false`, `clay schedules delete` is accepted; when `hasRun` is `true`, delete returns `conflict` (exit 1) and you must use `update` with `"status":"paused"` or `"status":"completed"` instead. A completed schedule can be revived by saving a new `scheduleConfig` with a future occurrence.
