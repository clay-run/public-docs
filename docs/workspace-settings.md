...
Workspace admins on the **Enterprise** plan can restrict which IP addresses are allowed to reach the workspace. Restrictions can be configured independently for browser sessions (web app) and API traffic, and are managed through a self-serve UI — no support request is required.

See [IP access restrictions](./ip-access-restriction.md) for a full setup guide, including how to create allowlists, enable restrictions per category, and manage lockout protection.
