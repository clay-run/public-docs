...
-   **LinkedIn URLs** — to find a person's profile URL when you already have their name and company (or email address), click `Add enrichment`, search for `LinkedIn URL`, and select the waterfall option under **Waterfalls**. The waterfall cascades through multiple providers and accepts an email address, or a name plus company name or domain as inputs. Note: some providers within this waterfall require Launch plan or above; Trial users who see a "subscription does not allow this integration" error should remove those providers from the waterfall sequence.
