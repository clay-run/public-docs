...
**To automate the straggler sweep:** Instead of running **Run N empty or out-of-date rows** manually after each batch, you can schedule a recurring retry. Open **Table settings** (⛭ icon) → **Run Settings** → enable **Re-run columns on a schedule**, use the column selector to include only the lookup column, and set the frequency. One tradeoff: a scheduled re-run fires for **every row** in the table each cycle — including rows that already found a match — and each of those rows is billed at the standard rate. The schedule cannot be limited to a filtered view of empty rows only. With Delay run in place, most rows match on the first try; the scheduled sweep is a safety net for stragglers whose external system creation lag exceeded your delay. Note that Hourly frequency requires an Enterprise plan; other plans have Daily as the most frequent option. See [Scheduled columns](scheduled-columns.md) for full setup details.

**Avoid using a time-based run condition as a substitute for Delay run.** If you add a run condition that evaluates to false when a row first arrives — for example, a formula checking how long since the record was created — the cell shows **"Run condition not met"** and will not automatically re-evaluate once the waiting period passes. Clay re-evaluates run conditions only when an upstream column value changes for that row (see [Conditional runs](conditional-runs.md)); a time-based check with no upstream dependency stays stuck. Use **Delay run** instead.
