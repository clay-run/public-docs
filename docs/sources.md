...
**Long text or AI column data appears truncated or shows `<object>` in the export.** If a column stores enrichment or AI-generated output as nested structured data — such as the output of a Use AI, Claygent, or enrichment integration column — the CSV export may show only a short preview ending in "..." or the placeholder text `<object>` rather than the full value. Both occur when the export encounters a structured object rather than a plain extracted field.
