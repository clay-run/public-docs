...
-   **Missing `items` on an array field.** Every field with `"type": "array"` must include an `"items"` object that specifies the element type. Without it, the AI provider rejects the schema before processing any tokens — all cells in that column immediately show the generic **"Error. Please retry or contact Clay support."** message, and no credits are consumed. To see the specific error, click any red cell to open the Cell Details panel; it shows: `Invalid schema for function 'returnData': In context=('properties', 'fieldName'), array schema missing items`. Fix it by adding `"items"`:
