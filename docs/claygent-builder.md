...
-   **"Failed to parse formula for 'prompt' with error: {}"** (the error detail may be empty `{}` or include a position like `{"pos":733,...}`) — The prompt field contains an invalid formula. Clay stores prompt text as a formula so that `{{column}}` references work, and if the formula has a syntax error, the column cannot run. Common causes: a stray backtick (`` ` ``), a straight double-quote character (`"`) appearing inside the prompt text that the formula parser reads as ending the formula string prematurely — for example, a phrase like „quoted word" where the opening mark is a typographic character but the closing `"` is a straight ASCII double-quote (this often happens when writing prompts in word processors or non-English keyboard layouts that use curly opening marks alongside straight closing marks); an unmatched brace; or characters introduced when copying text from a source that encodes content as HTML — for example, pasting from a chat window or email client that converts `<`, `>`, and `&` into HTML entities (`&lt;`, `&gt;`, `&amp;`).
...
**To fix the prompt formula error:** Open the column settings (**click the column name → Edit column**) and inspect the Prompt field. Remove any stray backticks, unmatched braces, or HTML entities. If your prompt text includes a straight double-quote character (`"`) as punctuation — for example, as part of a quoted phrase — replace it with single quotes (`'`) or rewrite the phrase to avoid the internal quote mark. If you copied the prompt from a chat tool, word processor, or email, paste it into a plain-text editor first to strip hidden formatting and typographic characters before pasting into Clay.
