...
**Note:** The **Account** dropdown and the option to use your own API key are available in both **table columns** and **Workflow nodes**. When running a Claygent inside a Workflow node, the Account selector appears in the node's configuration whenever the selected model requires an API key, so you can use either the Clay-managed key or your own private API key.
