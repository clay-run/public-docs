...
1.  Configure the source to your preferences:
    ...
    -   **Location** — Filter by company office location. Sub-filters: **Country**, **City**, **State or province**, **Region** (EMEA, NAM, APAC, or LATAM), and **Postal code**. Use **Is Headquarters** to restrict results to companies whose primary office is in the specified location. All sub-filters support include and exclude.
        ...
        **If your filter panel shows location conditions labeled "≥ 1 location is all of:":** each location block evaluates independently — a company clears a block if *any single office* satisfies all sub-conditions within that block, and two separate blocks can each be satisfied by *different* offices of the same company. To require a headquarters in a specific country, place the **Country** sub-filter and **Is Headquarters** inside the *same* block. If Is Headquarters is in a separate block from your Country filter, it only checks that the company has a headquarters *somewhere*, not in your specified countries. To add a sub-filter inside an existing location block without creating a new block: click the expand chevron (↓) on the right edge of the block, then select **Add filter** from the menu.
...
This applies if your filter panel shows location conditions labeled **"≥ 1 location is all of:"** — each such block evaluates independently, and two separate blocks can each be satisfied by *different* offices of the same company.

If **Country = Netherlands, United Kingdom** is in one location block and **Is Headquarters = true** is in a separate second block, a company like Procter & Gamble (headquartered in Cincinnati) passes because one of its offices is in the UK (clearing block 1) and its Cincinnati headquarters clears block 2 separately — even though the UK office is not the headquarters. The filter summary at the top of the panel will show this as two distinct location conditions: "With at least one office in the Netherlands or United Kingdom" and "With at least one headquarters location."
...
To move Is Headquarters inside an existing Country block: click the expand chevron (↓) on the right edge of your Country location block, then select **Add filter** from the menu. Add **Is Headquarters = true** inside that block. The filter summary will then show a single combined condition — "With at least one headquarters location in [your countries]" — confirming both conditions apply to the same office.
