...
MCP (Model Context Protocol) is how Clay connects your workspace to AI tools like ChatGPT, Claude, Glean, and Grok. Clay lets workspace admins set credit limits and monitor usage for team members who access Clay through these platforms.
...
**Note:** Clay MCP is currently in Open Beta. The admin credit-control features described below (default credit limit, per-user credit limit overrides, the `MCP users` table, and hard-blocking on limit reached) are gated behind a feature flag — contact support to enable them for your workspace.
...
Functions are reusable enrichment workflows built in Clay that reps can invoke directly from ChatGPT, Claude, Glean, or Grok with a single prompt. Admins build them once and enable them for MCP — reps never need to open Clay to use them.
...
Credit limits cap how many Clay credits a rep can spend through ChatGPT, Claude, Glean, or Grok in a given month. Credit spend resets on the 1st of each month at midnight UTC.

There are two levels of control:

-   **Default credit limit** — applies automatically to all new MCP users when they first connect ChatGPT, Claude, Glean, or Grok. Click `Set default limit` to configure. Reps without an individual override inherit this limit.
...
-   **Platforms** — icons indicating which platforms the rep has connected (ChatGPT, Claude, or Grok, or a combination). Reps connected through other platforms — including Glean — appear with a generic globe icon labeled "Connected to Unknown."
...
Any team member who needs to use Clay through an AI tool (Claude, ChatGPT, Glean, or Grok) must first be added to your Clay workspace. When inviting them, assign the **Sales Rep** role.
...
**Important:** Team members must accept their workspace invite email *before* connecting Clay to Claude, ChatGPT, Glean, or Grok. If a rep goes through the Clay MCP connection flow before accepting the invite, they'll be routed into a new personal workspace instead of your team workspace — and they won't appear in your `MCP users` table or be eligible for credit allocation.
...
-   **Grok:** In grok.com settings, find MCP integrations or connected apps and remove Clay.
...
Further actions through ChatGPT, Claude, Glean, or Grok are hard-blocked until the monthly reset — the rep won't be able to run enrichments or invoke Functions. Admins can increase the per-user limit at any time from the `MCP users` table to restore access immediately.
...
The default limit is a workspace-wide setting that applies automatically to any new rep who connects ChatGPT, Claude, Glean, or Grok. A per-user override replaces the default for a specific rep. Reps showing `No limit` have neither a default nor an override applied.
