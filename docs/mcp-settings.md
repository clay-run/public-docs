...
MCP (Model Context Protocol) is how Clay connects your workspace to AI tools like ChatGPT and Claude. Clay lets workspace admins set credit limits and monitor usage for team members who access Clay through these platforms.
...
-   **Restrict account querying by Salesforce owner** — when enabled, MCP users can only query accounts they own in Salesforce. Requires a Salesforce audience connection. The toggle is greyed out and inactive when Salesforce is not connected.
...
There is no generic Clay MCP server URL to enter manually. The "Add MCP server" configuration screen in third-party tools is for custom MCP servers — Clay's integration connects through each platform's built-in app directory, not that form.
...
**Important:** Team members must accept their workspace invite email *before* connecting Clay to Claude or ChatGPT. If a rep goes through the Clay MCP connection flow before accepting the invite, they'll be routed into a new personal workspace instead of your team workspace — and they won't appear in your `MCP users` table or be eligible for credit allocation.
