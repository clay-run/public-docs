...
-   When **disabled**: You must manually click cells to trigger enrichments. **Auto-run only controls enrichment columns — it does not affect your data sources.** Scheduled source imports (such as a Salesforce SOQL query or any source with an auto-update schedule) continue to add new rows to the table on their normal schedule even when Auto-run is off. To also pause row ingestion, disable the source's auto-update schedule separately in the source column settings.
    ...
    **Timing note:** Enrichments fire the moment rows arrive — if Auto-run was still on when a scheduled source last imported rows, those enrichments have already run and those credits have already been spent. Turning Auto-run off afterward does not retroactively cancel a run already in progress. To prevent unexpected credit usage from a scheduled source going forward: ensure Auto-run is off *before* the source's next scheduled run, or change the source's run mode from **On a schedule** to **Manually** in the source's **Run settings** panel. Switching to Manually stops the source from importing rows automatically, giving you full control over when new data enters the table and when enrichments fire.
