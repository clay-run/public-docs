...
-   **The column shows "✅ Sent" after re-running, but rows you deleted from the destination table do not reappear:** When Send Table Data sends a row for the first time, it creates a new row in the destination table and stores a reference to that destination row's ID inside the source column. On every subsequent re-run, the column uses that stored ID to update the same destination row — it does not create a new one. If you delete those rows from the destination table, the source column still holds the old (now-invalid) row IDs. Re-running reports success, but each update targets a row that no longer exists, so nothing appears in the destination table and no new rows are created. Force-rerunning or clearing the column has no effect because the stale row ID persists in the source column's stored state.

    **Fix:**
    1. Delete the Send Table Data column from the source table.
    2. Select **Tools → Export → Send table data**, reselect the destination table, and re-map your fields. The new column starts fresh — its first run will create new rows in the destination table.
    3. Before running the new column, enable [auto-dedupe](table-management-settings.md#auto-dedupe) on a unique identifier column in the destination table (such as email address or domain) to prevent duplicate rows if any records that were previously sent arrive alongside records that were never deleted.
