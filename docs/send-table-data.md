...
-   **The Send Table Data configuration shows a "Deleted fields — Action required" warning with one or more field names listed:** This warning appears when columns that were previously mapped in this action's configuration have since been deleted from the destination table. The action cannot route data to fields that no longer exist, and those stale mappings must be removed before the column can run successfully. Click the trash icon next to each field listed under "Deleted fields" to remove it from the configuration, then save. If those columns are still needed, recreate them in the destination table first and then re-add the field mappings in the Send Table Data configuration.
