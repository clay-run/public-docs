...
-   **'This field has already been added' error when selecting additional data fields:** This error appears when you select the same source field path twice in the configuration. Remove the duplicate entry to clear the error. Separately, if two different enrichments share a field name — for example, two ad integrations both producing a field named `results` — their formula paths are distinct and do not trigger this error. Instead, the destination column name for the second field defaults to blank: fill it in manually before saving to avoid a naming collision in the destination table.
