...
-   There can be a **maximum of 20 tables** connected via Send Table Data across your entire workspace, including tables in other workbooks. Connections made by **Write to Other Table**, **Execute Subroutine**, and **Route to Campaign** actions also count toward this same workspace-wide limit. If you exceed the limit, you'll see the error: *"Maximum number of table ID checks exceeded (limit: 20)"*. The limit counts unique destination tables reachable through the inter-table connection graph — not the number of Send Table Data columns. **Lookup Single Row in Other Table** and **Lookup Multiple Rows in Other Table** do not count toward this limit. To work within this limit:
    -   If you're using Send Table Data primarily to check whether a record exists in another table (for example, a "Do Not Contact" or "Recently Contacted" list), replace those connections with **Lookup Rows** — they return the same match result without consuming a connection slot. See [Lookup Rows](https://university.clay.com/docs/lookup-rows) for setup instructions.
...
-   **The destination table has reached its 50,000-row limit and is no longer accepting rows — how do I continue?** Each destination table can hold a maximum of **50,000 rows** from a Send Table Data source. When the table is full, new rows from the upstream Send Table Data action will be rejected. The most reliable approach is to route the remaining rows to a **new downstream table** with a fresh source.
    ...
    **Note:** The 50,000-row cap on Send Table Data destination tables applies regardless of plan. Passthrough mode (Enterprise) does **not** bypass this cap for Send Table Data destinations — its auto-delete behavior applies to webhook-sourced tables only, not to tables receiving data from a Send Table Data action. For continuous high-volume workflows that will routinely exceed 50,000 rows, the lookup-and-condition pattern above (routing remaining rows into additional downstream tables) is the recommended approach.
...
-   **`sourceId Required` error after copying a workbook:** When you copy a workbook that contains a Send table data column, the column's internal table reference may not update automatically to point to the newly created tables. Running the column in the copied workbook then fails with `Invalid send table data inputs … "sourceId" … "Required"`. You may also notice that running or duplicating the column sends data back to the *original* workbook's destination table instead of the copy's. **Fix:** Delete the Send table data column in the copied workbook and recreate it — click **+ Add column**, select **Send table data**, choose the destination table *in the copied workbook*, and save. This registers a fresh reference to the correct tables and resolves the error.
