...
-   **A field from a "rows from" cell doesn't appear as a selectable input when configuring an enrichment or function column.** When data arrives via Send Table Data, it lands in a "rows from: [source table]" cell. Because Clay has no predefined schema for what fields this cell may contain, the input picker discovers available fields by scanning the first **20 rows** of the current table view. If a field only has values in rows beyond the first 20 — for example, because only a small number of rows were re-run through Send Table Data after the field was added — it won't appear in the picker dropdown, even if it has data.
    ...
    **To fix this, use either workaround:**
    ...
    -   **Sort the view so that rows with the field value appear in the first 20 positions.** Sort the table by any column that brings rows containing your field to the top, then reopen the input configuration — Clay will scan the updated view and surface the field.
