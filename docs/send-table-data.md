...
-   **"✅ Sent" means Clay dispatched the data — not that it has appeared at the destination yet.** The `Sent At` timestamp and `Number Of Rows Sent` shown in the cell details confirm that Clay successfully placed those rows into the destination table's incoming source. The destination table still needs to process those rows, which typically takes only seconds. To verify data is arriving, open the destination table and check its [column progress bar](https://university.clay.com/docs/run-progress-ui).
