...
-   **`Send row for each item in a list` shows ✅ SUCCESS, but rows are missing from the destination table:** The source array for those rows was empty — it contained 0 items rather than being absent entirely. Clay iterates over the items in the list; an empty list produces 0 rows and still returns a success status with the preview text "✅ Sent 0 rows." Click the status cell to confirm: a **Number Of Rows Sent: 0** detail means the array had no items at run time. **Fix — two options:** (1) Add a [run condition](https://university.clay.com/docs/conditional-runs) to the Send Table Data column so it only runs when the list column is not empty — same approach as the `listData Required` fix above. (2) If you also need to send row-level fields that exist regardless of whether the array populated (for example, contact names, emails, or phones stored directly on the row), create a **separate** Send Table Data column using **Send row** and map those fields directly. That column sends one row per source row unconditionally, independent of any array column.
