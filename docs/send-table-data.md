...
**If the column holds a comma-separated text string** — for example, a value like `"CEO, CFO, CTO"` produced by an **Extract values from data** column — add a **Formula** column with the expression `{{Your Column}}.split(",")` to convert the text into a native array. Then select that formula column as the list input in **Send row for each item in a list**. Note that wrapping the column in brackets (e.g., `[{{Your Column}}]`) creates a single-element array containing the entire text string — it does **not** split the text into separate items.
