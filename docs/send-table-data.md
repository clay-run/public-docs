...
-   **You don't see a "Rows from: [source table name]" column in the destination table:** The column is created in the destination table as soon as you save the Send Table Data action — it does not require the action to have run yet. If the column hasn't appeared, refresh the destination table. In rare cases the column doesn't generate on save; if that happens, run a small test batch (5–10 rows) to confirm it appears before running the full table.
