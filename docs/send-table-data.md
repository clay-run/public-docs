...
**Object columns vs. list columns:** Not every enrichment column is a valid list source. Clay only accepts a field that holds an **array** — a cell containing multiple separate items, not a single structured record. Two types of enrichment columns exist:

- **Object columns** — such as **Enrich Company** or **Enrich Person** — return one structured record per row. Clicking into the cell details panel shows many nested fields, but the column holds a single item, not an array. Selecting an object column directly as the list source will show **"Please add a valid list."** because Clay has no array to iterate over.
...
-   **`Send row for each item in a list` shows "Please add a valid list." after selecting an enrichment column like Enrich Company or Enrich Person:** These columns return a single structured object per row, not an array of multiple items — even though clicking into the cell details panel reveals many nested fields. Clay requires the list source to hold an actual array. Use a column like **Find Contacts at Company → People** or **Find Active Job Openings → Job Openings** instead, as those return arrays. If you need to flatten a nested sub-array within an object column (for example, a list of past positions inside an Enrich Person result), expand the column in the token picker to find the specific nested array sub-field and select that instead of the parent column.
