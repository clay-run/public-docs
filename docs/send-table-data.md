...
**If the column holds a stringified JSON array** — for example, a text value that looks like `[{"name": "Alice"}, {"name": "Bob"}]`, common when data arrives from an HTTP API call or webhook — Clay won't recognize it as a native list and will show the same **"Please add a valid list."** error. To fix this, click the **gear icon** on the right side of the list input to switch to formula mode, then enter `JSON.parse(/YourColumn || "[]")`, replacing `YourColumn` with the name of your column (use `/` to reference it). The `|| "[]"` fallback substitutes an empty array for rows where the column has no value, so those rows skip cleanly instead of failing at runtime.

**Formula columns that return arrays** are subject to the same constraint. Formula columns use the Text type internally and cannot store native arrays — if your formula body returns a plain array of objects, the column errors out and the value is unavailable to other formulas referencing it. To pass a constructed array through a formula column into Send table data, explicitly convert it to a JSON string inside the formula using `JSON.stringify(...)`, then parse it back in the list field: switch to formula mode and enter `JSON.parse(/YourColumn || "[]")`.

**Syntax note:** The function is `JSON.parse` — capital `JSON`, lowercase `parse`, separated by a dot. `JSON_parse` (underscore) and `JSON.Parse` (capital P) are not valid in Clay formulas. Clay's formula engine does not throw an error for misspelled function names; instead they silently evaluate to `undefined`, which then fails the list validation at runtime with "expected array, received undefined."
