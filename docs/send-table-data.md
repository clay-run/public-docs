...
-   **Enrichment columns in the destination table will run on rows you send there — including when you use Send Table Data to snapshot a table.** When rows arrive in a destination table via Send Table Data, they are brand-new rows from that table's perspective — even if those records were already enriched in the source table. Any enrichment columns with **auto-run enabled** will fire on every incoming row and consume credits. Send Table Data copies each row's existing cell values from the source table without re-running source enrichments, but destination enrichments fire based on the destination table's own auto-run settings. This is a common source of unexpected credit spend when moving or reorganizing data across tables.

    **To snapshot a table and preserve enriched values without triggering destination enrichments:**

    1. Open the **destination table** and turn off auto-run on each enrichment column you don't want to re-run: click the column name → **Edit column** → toggle **Auto-run** off under Run settings → **Save**. Repeat for each enrichment column.
    2. Run **Send Table Data** from the source table. Clay copies each row's existing cell values as-is — source enrichments do not re-run at this step. If any destination enrichment columns still have auto-run on, Clay shows a **"Send data to table with auto-run on"** confirmation dialog — check the estimated cost before confirming.
    3. Once the rows have arrived, re-enable auto-run on the enrichment columns you want active going forward. Rows that arrived while auto-run was off will **not** automatically re-run when you re-enable — only rows added after that point will trigger those enrichments automatically.
