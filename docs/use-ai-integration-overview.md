...
**Important:** Connecting an API key to your workspace (via `Settings` → `Connections`) does **not** automatically update existing AI columns. Each column stores the account that was selected when it was configured, and adding a key to the workspace later does not change that saved selection. To use your own key on an existing column, open the column settings, click the **Account** dropdown, and select your connected account. If the Account dropdown still shows the Clay-managed account, your own key will not be active for that column even if it is connected to the workspace.
