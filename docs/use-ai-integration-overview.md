...
**Note:** The **+ Add account** option requires a direct **API key** from your AI provider (available from your Anthropic, OpenAI, or Google console). This is separate from any account you may have connected through Clay's **Settings → Connections** page — OAuth connections and Use AI API key authentication are independent. Even if you have already connected an Anthropic or OpenAI account as an integration, you still need to add your API key here for Use AI and Claygent to use your account.
