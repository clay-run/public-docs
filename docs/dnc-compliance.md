...
-   Check your phone number lists **against applicable national DNC registries** (e.g., in the U.S. National Do Not Contact Registry, in the UK, the Corporate Telephone Preference Service) to ensure numbers listed by consumers are excluded. We've partnered with [Meer](http://meerapi.com/) to help you easily do this within your existing Clay workflows. Meer currently supports US (National DNC Registry and select state registries), UK (TPS/CTPS), Ireland ([comreg.ie](http://comreg.ie/)), Belgium (DNCM), Germany ([Robinsonliste.de](http://robinsonliste.de/)), Spain ([Lista Robinson](http://listarobinson.es)), New Zealand, and Australia, with additional countries being added regularly.
    ...
    -   **Recommended workflow (enrich → screen → gate):** Meer screens phone numbers, not names — a phone number must exist in your table before you can screen it. Set up your columns in this sequence:
        ...
        2.  Add a Meer DNC-screening column **downstream** of the phone column.
        3.  Set an **"Only run if"** condition on your CRM-sync or dialer export action so that only contacts where **Do Not Call = false** are exported — this gates DNC-flagged numbers out **before** they reach your system of record, not after. **Do Not Call = false** means only that the number was not found on a covered suppression list — not that it is legally eligible for outbound calling. Apply your own country-level calling rules and risk policies before routing numbers into call-first sequences, especially in markets such as Germany where additional requirements (for example, presumed consent for B2B telephone advertising) may apply beyond registry absence.
        4.  Maintain your internal opt-out/suppression list as a Clay table and add a lookup step to suppress contacts who have asked not to be contacted — internal suppression is required in addition to national registry screening.
...
-   **For contacts in countries where Meer does not yet have registry coverage**, use the following strategy:
    -   **Internal opt-out list as your global baseline:** Maintain a company-wide opt-out list synced into Clay to suppress contacts globally, regardless of country. This is the most important compliance layer for markets without a Meer-covered registry. See Section 5 (How Clay Supports Compliant Outreach) below for setup steps.
    -   **Geo controls via conditional logic:** Use Clay's "Only run if" conditions and selective CRM upload settings to treat contacts from different regions differently — for example, restricting CRM sync for regions where you have not yet established a compliant outreach workflow.
    -   **EU countries outside Meer's coverage (e.g., France, Netherlands):** GDPR requires you to document a lawful basis for processing personal data before using phone numbers for outreach. Clay's conditional logic can help enforce geo-based gates at the data layer, but documenting the lawful basis is your responsibility.
    -   **APAC, LATAM, and Middle East:** Regulations vary significantly by country. Consult your legal team to define compliant outreach boundaries per market — Clay can then enforce those rules via workflow logic.
