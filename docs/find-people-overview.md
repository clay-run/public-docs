...
> **Important:** These actions require a standard LinkedIn post URL. Any URL containing `-activity-`, `-ugcPost-`, or `-share-` in the path (e.g., `https://www.linkedin.com/posts/clay-hq_...-activity-7212099008951975937-ezPv`) or a direct `feed/update` URN URL (e.g., `https://www.linkedin.com/feed/update/urn:li:ugcPost:7264751670859911168`) is valid. Non-post LinkedIn URLs — profiles, company pages, or article links — are not valid and return an error. To get the URL for any post: open the post on LinkedIn, click **•••** (three dots) at the top right, and select **Copy link to post**.
