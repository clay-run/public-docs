...
**Note:** If you delete rows that were previously imported by a Find People source and then re-run the source, those contacts will not be re-imported. The source tracks all previously returned results in an internal exclusion list; re-runs skip anyone already seen, regardless of whether their row was later deleted from the table. To retrieve the same contacts, add a new Find People source to the same table with the same search settings — a new source starts with a fresh exclusion list and will re-import all matching contacts.

**Note:** Selecting **Replace existing results in table** when re-running a source permanently deletes all rows currently in the table and imports only the contacts matching the source's current filters. The source's import history is also reset to track only these newly imported contacts. If you need to recover rows removed by a Replace: edit the source to restore your original search criteria, then re-run it and select **Add new results to table** — or add a new Find People source to the same table using the original filters. To narrow a large list without losing rows, use a **View** on the table instead of editing your source filters: create a view and apply filters there by job title, company, or any other column. Views show only a subset of rows without modifying the underlying table, and you can adjust or clear those filters at any time.
