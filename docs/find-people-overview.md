...
-   **Companies:** Find people at specific companies using an existing Clay table or a custom list. By default, this matches people who **currently** work at those companies.
    -   **Custom list:** Enter company identifiers directly — no separate Clay table required. Accepted formats: company domains (e.g., `vestas.com`), LinkedIn company page URLs (e.g., `https://www.linkedin.com/company/vestas`), Sales Navigator URLs, and Sales Navigator company IDs. See [company identifier best practices](finding-companies-and-people-in-clay.md#use-linkedin-urls-not-domains-as-company-identifiers) for guidance on which format gives the most accurate results.
