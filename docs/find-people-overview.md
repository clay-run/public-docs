...
**Exclusion matching is based on LinkedIn profile URL only.** Clay extracts the profile slug — the identifier after `/in/` in the URL — from each row in your exclusion table. Common format variations like `www.` prefixes, trailing slashes, and query parameters are normalized automatically, so `https://www.linkedin.com/in/john-doe/` and `linkedin.com/in/john-doe` are treated as the same person. Email addresses in your exclusion table are not used for matching.

The most common cause of slip-throughs is a **LinkedIn vanity URL change**: if a person updated their LinkedIn slug (for example, from `john-doe-12345abc` to `john-doe`) after you built the exclusion list, the old URL in your exclusion table no longer matches their current profile, and Clay will not exclude them.

**To troubleshoot:** Check whether the LinkedIn URL in your exclusion table matches the person's current LinkedIn profile URL. Update the row with their current URL and re-run the source.
...
**LinkedIn Sales Navigator company searches are not supported by this source.** Company search URLs (`https://www.linkedin.com/sales/search/company/...`) are rejected — there is no equivalent import source in Clay for Sales Navigator company lists. To bring a company list from Sales Navigator into Clay, export it from Sales Navigator as a CSV and [import the CSV into Clay](csv-import-overview.md) instead. You can then enrich the imported companies using Clay's company enrichment sources.
