...
-   **Company domains or profile URLs (optional):** Scope results to posts by or about specific companies. Enter up to 5 company domains or LinkedIn company profile URLs — only the first 5 are used. Enter values manually or use the **From table** tab to pull from a separate Clay table.
-   **People filter (optional):** Limit results to posts that mention or were posted by specific individuals.
-   **Person professional profile URLs (optional):** Scope results to posts by specific people. Enter up to 5 LinkedIn profile URLs — only the first 5 are used. Enter values manually or use the **From table** tab to pull from a separate Clay table. **Note:** Source inputs do not support dynamic column references — you cannot point this field to a column in the same table. To target profiles from an existing table, write the LinkedIn profile URLs to a separate table first and link it using the **From table** option.
...
**To get all interactions on a specific post:** Use the **Get interactions with professional posts** source. In a workbook, click `+ Add`, search for `Get interactions with professional posts`, and select it. Map the post URL column from your import. This returns a row per interaction (comments, reactions, and shares combined).
