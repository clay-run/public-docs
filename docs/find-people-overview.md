...
-   **Limit results:** Set a maximum number of results per search (up to 50,000 records).
    -   **Free plan note:** The free plan limits each table to 200 rows. If **Limit results** is set above 200, the source cannot run at all — set it to 200 or fewer to allow the search to work on the free plan.
