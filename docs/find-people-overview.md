...
-   **Exclude people:** Exclude up to 3 different sets of people from your search using Clay tables, People segments, CSVs, or manual lists. You can exclude up to 300,000 people total (100,000 per source). Exclusions match by **LinkedIn URL or email** — each row in your exclusion source must contain a person-level LinkedIn URL or email address. Adding a company name, domain, company page URL, or job title to the exclusion source will **not** suppress those people from the results. Available on Launch plan and above — contact support if the **Exclude people** option isn't visible in your workspace.

    **Segment exclusions are evaluated dynamically at run time.** If you use a People segment as an exclusion source, the segment membership is re-evaluated on each run — the exclusion list is not locked to who was in the segment the first time the source ran. People who enter the segment after the initial run will be excluded on subsequent runs; people who leave the segment will no longer be excluded on subsequent runs.
