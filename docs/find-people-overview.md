...
-   **Companies:** Find people at specific companies using an existing Clay table, a Company Audience, or a custom list. By default, this matches people who **currently** work at those companies.
    ...
    -   **Using a Company Audience:** When you select a Company Audience as the companies source, no **Company identifier type** selector appears — this is expected. Clay has already resolved each company record in the Audience to an internal company ID using both its company profile URL and domain (via entity resolution), so there is nothing to choose at search time. Find People then matches people against those pre-resolved IDs. For the most accurate matching, make sure your company records in the Audience have a company profile URL populated: a profile URL maps directly to a single company, whereas a domain alone can occasionally resolve to a related or parent entity and return fewer results.
