...
-   **Companies:** Find people at specific companies using an existing Clay table, a Companies Audience segment, or a custom list. By default, this matches people who **currently** work at those companies.
    -   **Locked after creation:** The filter type (Clay table, Audience segment, or custom list), which table or segment is linked, and — when using a custom list — the specific company identifiers entered cannot be changed once the source exists. This is what the "Can only be changed during source creation" tooltip means. If you need to target a different set of companies (for example, to correct a mistyped domain), create a new Find People source with the corrected identifiers. The linked Clay table's or Audience segment's contents are not frozen, however: adding or removing rows from the linked table, or adding or removing companies from the linked segment, is reflected each time the source runs. When re-run after new companies are added, the source searches across all companies currently in the table or segment, not just the newly added ones.
