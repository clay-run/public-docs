...
Looking to monitor a specific enrichment? Learn how to create Custom Signals.
...
6. Optionally, `Add sample results` to preview how the data will appear after changes actually happen.
7. Click `Save and run X rows` to finish the Signal.

### Edit an existing Signal

1. Click on the column title with the Signal (it'll have a 📡 icon and usually be called `Event`).
2. Click `Edit column`.
...
Most Signals are available on any paid plan. The LinkedIn social listening Signal is only available to Pro and Enterprise customers.
