...
The Clay email sequencer requires a paid plan or active trial — it is not available on the Free plan. Free plan workspaces see an upgrade prompt when accessing Campaigns. Each lead sequenced consumes 1 Action (platform orchestration work). If you use AI snippets in your messages, those consume 1 Action per run and Data Credits for AI generation in addition to the Action for sending the email.
