---
title: Workflow FAQs
description: "Answers to common questions about Workflows access and plan availability, list iteration, run behavior, costs, and how Workflows integrates with Audiences and tables."
last_synced: 2026-09-04T05:06:12.203Z
---
...
Access to Workflows itself is separate from your role: during open beta you turn it on by enrolling your workspace in the beta program from your workspace settings. If Workflows isn't in your left sidebar under `Orchestration` at all, check your enrollment there first. Your Growth Strategist or Clay support can help if it still doesn't appear.

### Which plans include Workflows?

Workflows is available on every plan, including free and trial plans. On the modern Enterprise, Growth, and Launch plans it's available indefinitely, and legacy Enterprise customers keep it indefinitely as well — the time limit below applies only to the legacy self-serve plans.

What varies by plan is what you can put in a graph. The free plan includes rules-based conditionals but not `Run code` or the `Code` and `AI` conditional modes, and some triggers need a paid plan — those stay visible on the canvas with an `Upgrade` badge rather than being hidden.

Starter, Explorer, and Pro have full access for a limited time. After that, Workflows on those plans moves to a free-plan experience rather than switching off, with limits on runs and run history alongside the free-plan exclusions above. The workflows you built and the data they've written back to Audiences stay either way, so moving to a modern plan is what restores full access. Your Growth Strategist or Clay support can tell you where your workspace stands.
