...
Trial tables can hold up to **1,000 rows each**. The table view also displays only the first **50 rows** — rows beyond that are blurred in the UI until you upgrade to a paid plan. When you export to CSV, only these first 50 rows are included in the download. Columns that push rows to external tools — including email sequencer integrations — also run only on the first 50 visible rows; rows beyond the display cap are not processed and will not be sent to your sequence. Bulk data export to Google Sheets is not subject to this limit and includes all rows in the table.
