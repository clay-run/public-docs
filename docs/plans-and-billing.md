...
-   **Purchase a one-time top-up** — available on paid plans only; not available on the free plan. Only workspace admins can make the purchase. For emergency needs during your billing cycle at a 30% premium (50% on legacy plans), subject to rollover limits. Go to `Settings` → `Usage` and click `Add one-time data credits`.
