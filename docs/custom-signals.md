...
3.  Set up a [scheduled run](https://www.clay.com/university/guide/scheduled-columns) for this lookup by clicking `⚙️` in the bottom right corner → enable `Re-run columns on a schedule`.
    ...
    -   **Note:** On non-Enterprise plans, you can't set a specific time for a scheduled run — it runs relative to when you first enabled the schedule. To avoid conflicts, ensure a short delay between when you enable each table's schedule. Enterprise customers can set a specific day of the week, time of day, and timezone for scheduled runs; see [Scheduled columns](https://www.clay.com/university/guide/scheduled-columns) for details.
