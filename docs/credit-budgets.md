...
Credit budgets appear at `Settings` → `Budgets`. Only workspace admins on Enterprise plan workspaces can create and manage budgets. The **Budgets** page is not visible to non-admin members or to workspaces on plans other than Enterprise — if you don't see it in Settings, you are either not a workspace admin or not on the Enterprise plan.

**Seeing a "Budget Credit Limit Reached" error in an AI column?** That error comes from a separate per-row credit cap inside the AI column's settings, not from a workspace budget. To fix it, open the AI column's settings and increase the **Clay Credit Budget** value. See [Use AI](./use-ai-integration-overview.md) for step-by-step instructions. Any workspace member who can edit the column can adjust this setting.
...
|---|---|---|
| **Credit budgets** | Named credit pools — assign workbooks to a budget and track spend across teams. Enterprise workspace admins only. | `Settings` → `Budgets` |
...
| **Clay Credit Budget (per AI column)** | Per-row credit cap on a single AI column — limits how many credits a single row can spend. Available to all workspace members who can edit the column. | AI column settings → **Clay Credit Budget** |

The first two features are complementary for Enterprise admins: use spend limits to cap individual workbooks while also assigning them to a budget for team-level cost attribution. The per-column Clay Credit Budget is independent and available regardless of plan or role — it prevents any single row from consuming more credits than the set limit.
