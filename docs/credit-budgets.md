...
When a workbook (or other resource) is assigned to a budget, all credit-consuming actions in that resource count against the budget's balance. If a budget runs out of credits, credit-consuming runs in assigned resources stop and display a **budget limit reached** message. The workbook itself remains fully editable — no data is lost and non-credit-consuming actions continue. To resume credit-consuming runs, an admin can increase the credit limit, reset the budget's spend, or reassign the workbook to another budget with available credits before rerunning the blocked enrichments.
