...
Your actual ad reach is based on the audience size (unique contacts), not the matched count. When this behavior causes the calculated match rate to reach 95% or higher, Clay displays the rate as **95%+** rather than an exact percentage on the professional network, Meta, Reddit Ads, Bing Ads, and Vibe — because the raw rate can exceed 100% when matched count exceeds contacts sent. A tooltip on the 95%+ display confirms this is happening for that run.
