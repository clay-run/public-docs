...
Each ad platform enforces its own minimum matched-contact threshold. LinkedIn requires 300 matched members; Meta requires 1,000 matched contacts. The most common cause is that no email column was mapped in the field mapping — ad platforms match contacts by email, so without it the platform processes all sent records but matches 0.
