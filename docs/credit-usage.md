...
-   **Workflows** — shows credit spend broken down by workflow. Filter by `When`, `Integrations`, or `Owner` and click `Apply filters` to narrow results. Sort by `Name` or `Credits used`. Historical data is available from May 7, 2026.
