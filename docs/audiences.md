...
**Note:** A search import populates field values (such as Industry) only for companies that are **new** to your Audience. For companies already in your Audience from Salesforce or Snowflake, their existing field values take precedence over Clay's search data and are preserved unchanged. To update existing records with Clay's industry or other search data, bring the search results into a Clay table and use `Upsert Audiences Record` to push those values to matching records.
