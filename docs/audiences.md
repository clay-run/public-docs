...
3.  Select `Criteria` and then add a `Filter` or `Filter group`.
    -   **Filters** evaluate a single condition at a time. Top-level filters default to AND — a clickable connector between them lets you switch the top-level logic to OR, the same way you can inside a filter group.
    -   **Filter groups** combine multiple conditions using their own AND/OR logic. Within a single filter group, all conditions share the same operator — all AND or all OR; changing the operator in the group header switches all conditions at once. To mix AND and OR, use a nested filter group. For example, to build `A AND B AND (C OR D)`: add A and B as top-level filters, click **`+ Filter group`**, then add C and D inside the group. Once the group contains two or more conditions, a small **`and`** button appears between them — click it to switch to **`or`**. To build `(A OR B OR C) AND D = false` — for example, to include accounts that match any of several criteria but exclude any that are also marked with a disqualifying attribute — click **`+ Filter group`**, add A, B, and C inside it, then click the **`and`** connector between conditions to switch them all to **`or`**. Add D = false as a separate top-level filter outside the group. With the top-level connector left at AND (the default), D = false applies as a global exclusion: an account can match any OR condition in the group and still be removed from the segment if it also satisfies D = false. The segment re-evaluates automatically as field values change — no static exclusion list required.
