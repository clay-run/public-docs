...
**Salesforce Leads (vs. Contacts):** The steps above apply specifically to records imported from Salesforce **Contacts**. Leads imported into Audiences do not carry Account IDs in the same way — the Account IDs field on a Lead record in Audiences may be empty. To access Company-level data for a Lead, use the Lead's email or professional network URL to look up the matching Company record in a separate `Lookup in Audiences` column.

### How do I look up a record in Audiences that may be missing an identifier field?

If a record in your Audiences is missing its email address or professional network URL — the two primary identifiers used for person-level `Lookup in Audiences` calls — you can still find and work with it using the Audiences record ID.

Every record in Audiences has an internal record ID, visible in the record detail panel when you open a record from the People or Companies view. This ID is stable and unique, and it is the most reliable field to use as the match key in `Lookup in Audiences` when standard identifiers are absent.

**To look up a record by Audiences record ID:**

1.  In a Clay table, add a `Lookup in Audiences` column.
2.  Set **Object type** to **People** or **Companies** as appropriate.
3.  Under **Fields to filter by**, select **Audiences record ID** and map it to the column in your table that holds the Audiences record ID.

This bypasses the email/URL matching entirely and returns the exact record with that ID.

**If you need to find all records missing a specific identifier field** (for example, all People records with no email): In your People audience, add a filter on **Email** → **is empty**. This returns all records without an email address. You can save this as a named segment and run bulk enrichments or take other actions on those records from there.
...
### What is the "Fin awaiting teammate input" conversation attribute?

When Clay's support AI (Fin) encounters a question it cannot answer from the available help documentation — or when it needs additional context from the Clay team to resolve a customer's issue — it sets the **"Fin awaiting teammate input"** attribute on the conversation. This attribute is an internal signal for the support team, not something customers see directly. It means the conversation requires a human review: either to provide an answer Fin could not find, or to add documentation so Fin can handle similar questions in the future.

### How do I look up which Audiences field identifier or record a specific Salesforce ID belongs to?

Clay uses the Salesforce record ID (for example, a Contact's `003…` ID or an Account's `001…` ID) as the external record identifier when syncing data between Salesforce and Audiences. Each Salesforce record that syncs into Audiences is stored with that Salesforce ID as its external identifier.

**To find the Audiences record for a known Salesforce ID:**

1.  In a Clay table, add a `Lookup in Audiences` column.
2.  Set **Object type** to **People** (for Contact/Lead IDs) or **Companies** (for Account IDs).
3.  Under **Fields to filter by**, select **Salesforce record ID** and map it to the column in your table that holds the Salesforce ID.

The lookup returns the matching Audiences record — including all enriched fields, segment memberships (as a filter option), and the Clay record ID.

**If the Salesforce ID lookup returns no result:** The record may not have synced yet (check import status), the Salesforce record may have been hard-deleted (only visible after the next weekly full sync), or the ID may belong to an object type not yet connected to Audiences (for example, a Salesforce Opportunity that doesn't have a matching company record). Contact Clay support if the record should be present but is consistently not found.

### How does the Audiences record limit work for Launch plan workspaces?

Launch plan workspaces have access to core Audiences features — importing via CSV, people/company search, and Clay table sends — but the record limit and source restrictions differ from Growth and Enterprise plans.

The Audiences record limit applies to the total number of unique records stored across All People and All Companies. Launch workspaces are subject to a lower record cap than Growth plans. For the current Launch plan record limit, check your workspace's **Settings → Plan** page — the limit is displayed there alongside your current record count.

**Sources available on Launch:**

-   CSV imports
-   People/company search (Find People, Find Companies)
-   Clay table sends (Continue → Save to People/Companies, or Upsert Audiences Record)

**Sources not available on Launch:**
...
To connect a CRM or data warehouse as an Audiences source, upgrade to the Growth plan or above.
