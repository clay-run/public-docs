...
**Salesforce record subset import (early access):** If your workspace has this feature enabled, a new **Salesforce import wizard** lets you choose how many records to bring into Audiences. After selecting a Salesforce object type (People, Accounts, Leads, or Opportunities), you can choose **All records** to import every record or **Record subset** to import only records matching a custom SOQL query. The Record subset option opens a SOQL editor where you enter your query, name the subset, and preview the matching records before confirming. This lets you import a targeted slice of Salesforce data — for example, only Contacts from a specific region or Accounts above a revenue threshold — rather than importing your entire object. Record subset import is currently in early access — contact your Growth Strategist to enable it for your workspace.
