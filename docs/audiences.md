...
**If a newly added field is still showing as `false` or empty in Clay:** The 24-hour export pushes whatever value a field holds in Clay at the time it runs — if a bulk enrichment that populates the field is still in progress (or hasn't been started yet), the export will push those in-progress or default values. There is no need to reconnect or reconfigure the sync. Wait for enrichment to finish and the field to hold its correct values; the next 24-hour export will then carry those values to Salesforce.

### How can I export records to Salesforce immediately without waiting for the 24-hour sync?

There is no self-serve option to trigger an on-demand export from the Audiences UI — the Audiences → Salesforce export runs on a fixed 24-hour schedule per workspace. If you need to push enriched data to Salesforce immediately — for example, after completing a bulk enrichment and before the next scheduled export — use a **Salesforce Update Record** action column in your bulk enrichment table instead.

See [How do I write enriched fields back to existing Salesforce records from a bulk enrichment?](#how-do-i-write-enriched-fields-back-to-existing-salesforce-records-from-a-bulk-enrichment) for full steps. This approach writes to Salesforce directly as part of the enrichment run, bypassing the 24-hour export cycle.

### How do I replace a CSV import with updated data?

CSV imports are one-time — they don't re-sync automatically. If your original CSV contained errors and you need to start fresh, the recommended flow is to archive the affected records first, then import the corrected file.

**Step 1: Archive the records from the original import**

1.  In Audiences, add a filter for **Person source** (for People) or **Company source** (for Companies) and select the CSV import you want to replace. This scopes your view to only the records from that import.
2.  Select all filtered records and click **Archive** to remove them from your Audience.

**Step 2: Import the corrected CSV**

1.  Click **Add data** → **Add Source** → select **CSV**.
2.  Upload your corrected file and complete the import as usual.

**Note:** Archiving the original records is optional if the corrected file only *adds* new records. However, if the corrected file replaces or updates existing records (for example, fixing email addresses or domains), archiving first ensures those old records don't remain in your Audience after the new import.
...
To archive (remove) these records from your Audience:

1.  Add a filter on **Source status** → **Deleted in source** to your segment. This surfaces only records whose Snowflake association has been removed.
2.  Select all filtered records and click **Archive**.

The records are removed from all Audience views. If the same record exists in another connected source (for example, Salesforce), it will remain visible in your Audience under that source — archiving removes the record from all sources simultaneously.

### How do I remove records from an audience?

To remove records from your Audience, **archive** them. Archiving permanently removes the record from all Audience views — it will no longer appear in Any People, All Companies, or any segment.

**To archive individual records:**

1.  In the record detail panel, click the **⋮** (three-dot) menu in the top right.
2.  Select **Archive**.

**To archive records in bulk:**

1.  Filter your audience to the records you want to remove.
2.  Select all visible records (checkbox in the column header).
3.  In the left sidebar, click the **⋮** (three-dot) menu next to the segment's name.
4.  Click **Archive all [N] records**.

Archiving is permanent and cannot be undone from the UI. Once a record is archived, it will not re-appear even if its source (Salesforce, Snowflake, etc.) still contains it — you would need to re-import it to restore it.

**Note:** Archiving removes records from Audiences only — it does not delete or modify records in your connected CRM (Salesforce, HubSpot) or data warehouse (Snowflake, BigQuery). CRM and warehouse records are unaffected by archiving in Clay.
