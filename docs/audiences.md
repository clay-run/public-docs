...
1.  In Audiences, go to **People** or **Companies**.
2.  Click **+ Filter** and add a filter on **Sync status → is → Deleted in source**.
3.  (Optional) Add additional filters to narrow to the specific Snowflake source, for example:
    -   **Origin source → contains → [your Snowflake source name]**
...
4.  Click **+ Create Audience** to save the filtered view as a new named segment.
5.  Once the segment shows the correct orphaned records, click the **⋮** (three-dot) menu next to the segment name and select **Archive records**.
...
BigQuery import behavior mirrors Snowflake when a record is no longer returned by your SQL query — Clay marks the record's BigQuery source association as **Deleted in source** during the next full sync. The Audience record is **not automatically removed**. Follow the same cleanup steps described in [How do I archive records that no longer match my Snowflake import query?](#how-do-i-archive-records-that-no-longer-match-my-snowflake-import-query) above, substituting your BigQuery source name in the **Origin source** filter.

### How do I replace a CSV import with corrected data?

See [How do I replace a CSV import with updated data?](#how-do-i-replace-a-csv-import-with-updated-data) above — the steps are the same whether you're correcting errors or refreshing with updated records.

### How do I archive records from a Clay table that was removed?

When a Clay table source is removed from Audiences, the records it contributed are marked with a **Deleted in source** status on their source association — but the Audience records themselves are **not removed**. To clean them up:

1.  In Audiences, go to **People** or **Companies**.
2.  Click **+ Filter** and add a filter on **Sync status → is → Deleted in source**.
3.  (Optional) Add a filter on **Origin source → contains → [your table name]** to scope to that specific table's records.
4.  Click **+ Create Audience** to save the filtered view as a named segment.
5.  Click the **⋮** menu next to the segment name and select **Archive records**.

### Why do some records in my Audience have a "Deleted in source" status?

**Deleted in source** appears on an Audience record when the source that contributed that record — a Salesforce object, Snowflake row, BigQuery row, HubSpot record, or Clay table row — is no longer present in that source.

Common causes:

-   **Salesforce:** The record was deleted in Salesforce (soft-deleted records are marked after the next 15-minute incremental sync; hard-deleted records are marked after the next weekly full sync).
-   **Snowflake or BigQuery:** The record is no longer returned by your SQL query — either removed from the underlying table, or excluded by a query change.
-   **Clay table:** The row was deleted from the source table, or the table itself was removed.
-   **HubSpot:** The record was deleted or archived in HubSpot and picked up during the next sync.

**The Audience record itself is not removed** — it persists with **Deleted in source** status, which you can filter on in any segment. To clean up these records, archive them — see [How do I remove records from an audience?](#how-do-i-remove-records-from-an-audience).

If you believe a **Deleted in source** status is incorrect — for example, the record still exists in Salesforce — the most common cause is that the record was hard-deleted and recreated in Salesforce with a new ID, or a mapping change caused the record's identifier to no longer match. Contact Clay support if you need help resolving a specific case.

### Why are records from my Audiences showing as duplicated in Salesforce?

Audiences deduplication happens in Clay only — Audiences does not merge, modify, or deduplicate records in your connected Salesforce org. If two Salesforce records for the same entity (for example, two Contact records for the same person) already exist in Salesforce before you connect Audiences, they will appear as two separate records in your Audiences' source data and may be exported back to Salesforce as two separate contacts.

Clay's entity resolution may merge them into a single Audiences record (based on email, LinkedIn URL, or probabilistic matching), but the export writes to whatever Salesforce record IDs are associated with those source entries — so both records still exist in Salesforce.

**To prevent Salesforce duplicates during export:**

-   Configure Salesforce's native duplicate rules to block or merge duplicate records before Clay's export lands.
-   Use Salesforce's duplicate management (Matching Rules and Duplicate Rules) to detect and merge duplicates on the Salesforce side.

Clay's export does not create net-new Salesforce records unless the `Create new Salesforce records` toggle is explicitly enabled — see [How do I create new Salesforce Accounts or Contacts from an Audience?](#how-do-i-create-new-salesforce-accounts-or-contacts-from-an-audience) above.

### Why does archiving a record in Audiences not remove it from Salesforce?

Archiving an Audience record is a Clay-side operation only — it removes the record from your Clay Audience segments, enrichments, and workflows. It does **not** modify, delete, or archive the corresponding record in Salesforce. The Salesforce record remains untouched.

If you need to delete or archive a record in Salesforce, do so directly in Salesforce. Clay's next incremental sync will then pick up the deletion and mark the Audiences record as **Deleted in source** (for soft-deleted records) or during the next weekly full sync (for hard-deleted records).

The reverse is also true: deleting a record in Salesforce does not archive or remove it from your Audiences. Clay marks it **Deleted in source**, but the Audience record persists unless you archive it manually.

### Why is my Audiences record count higher than my Salesforce contact count?

Several factors can cause your Audiences record count to exceed the number of active records in Salesforce:

-   **Multiple object types imported:** If you import both Contacts and Leads from Salesforce, records from both object types contribute to your People audience count — including Leads that have not yet been converted to Contacts.
-   **Records from other sources:** CSV imports, Snowflake or BigQuery imports, Clay table sends, and Find People searches all add records to your Audience independently of Salesforce. Your total Audiences count reflects all sources combined.
-   **Deleted in source records:** Records marked **Deleted in source** are not automatically removed from Audiences — they persist until you archive them. If you've deleted contacts in Salesforce over time without archiving the corresponding Audiences records, those "deleted" records continue to count toward your total.
-   **Entity resolution mismatches:** If entity resolution has not yet matched a Lead and a Contact as the same person (for example, because neither has a LinkedIn URL or email match), they appear as two separate records in your Audience even if Salesforce treats them as one converted contact.

To reduce your Audiences record count, archive the records you no longer need — see [How do I remove records from an audience?](#how-do-i-remove-records-from-an-audience). For **Deleted in source** records specifically, filter on **Sync status → Deleted in source** and archive those records to bring your count in line with your active Salesforce data.

### Why do archived records in Audiences still contribute to my CRM/DWH record count against my plan limit?

Archiving a record removes it from active segments, enrichments, and workflows — but archived records are still counted against your plan's CRM/DWH record limit. The limit tracks the total number of records that have been imported from CRM or data warehouse sources into your Audience (including those currently archived), not just the records actively visible in segments.

If you are approaching or have exceeded your plan's record limit and want to reduce the count, contact Clay support. There is no self-serve option to permanently delete records from Audiences and free up plan quota.

### Why does my People count in Audiences differ from the number of contacts shown in individual source filters?

The **All People** view shows every person record in your Audiences workspace regardless of source. When you filter by a specific source (for example, **Person source → is → Salesforce Contacts**), the count reflects only records attributed to that source.

A single Audiences record can have contributions from multiple sources — for example, a contact synced from both Salesforce and a Clay table send. That record appears once in the **All People** count but is counted under each source it belongs to when you apply individual source filters. This is expected behavior — it means the sum of per-source counts will often exceed the total **All People** count.

Additionally, not all records in **All People** are attributed to a named source filter option. Records imported via `Upsert Audiences Record` from a Clay table action do not create a named source entry and won't appear under a **Person source** filter — see [Why doesn't my Clay table appear in the Person source filter?](#why-doesnt-my-clay-table-appear-in-the-person-source-filter) for details.

### Why did archiving records from one source also affect records contributed by other sources (Salesforce, CSV, etc.)?

Archiving is applied at the **record level**, not the source level. When you archive a record — for example, by running **Archive records** on a segment filtered to records from a specific CSV import — you are archiving the entire Audiences record, including all the field values and source associations it has accumulated from Salesforce, Clay table sends, and other connected sources (Salesforce, CSV, HubSpot, Snowflake, BigQuery).

This means: if a record exists in both a CSV import and Salesforce, archiving it via the CSV-filtered segment will also remove it from your active Salesforce-sourced view. The record is not removed from Salesforce — it still exists there — but its Clay Audiences representation is archived.

**To avoid this:** Before archiving, verify your segment truly isolates only the records you intend to remove. For records with multiple source contributions, check whether the record should remain active in Audiences for its other sources, and if so, do not archive it — instead, contact Clay support to discuss options for removing specific source associations without archiving the entire record.

**Archiving records from your Audience does not affect the records in connected sources** (Salesforce, HubSpot, CSV, Snowflake, BigQuery). They remain in their original sources even after being archived. Archiving a record also does not prevent it from being re-imported if a future sync or import re-introduces it — re-importing an archived record's identifier is silently skipped, and the record stays archived.