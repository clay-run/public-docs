...
**Note:** A field that appears numeric may have been imported as text (shown by a T icon rather than #). Text fields — such as "Annual revenue range" synced from Salesforce as a string — will not show range operators. To fix this, click `Settings` in the bottom left of the Audiences view, find the field, and change its type to `#` (Number). Range operators will then appear when you add a filter on that field.
