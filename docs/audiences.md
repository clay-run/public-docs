...
**Note:** Databricks import for Audiences is available to Enterprise plan customers. Contact your account team or [support](https://www.clay.com/support) to request access.

1.  Click `Add data` → `Add Source` → select your [**Databricks integration**](https://university.clay.com/docs/databricks-integration).
    -   If you haven't connected Databricks yet, click `+ Add account` and complete the setup. See the [Databricks integration](https://university.clay.com/docs/databricks-integration) for setup instructions.
2.  Enter a SQL `SELECT` query to define which records to import.
    -   Click `Test` to preview your data before continuing.
3.  Confirm the preview looks correct, then click `Continue`.
4.  Define the `Unique Identifier`:
    -   For People: `email` or `user_id`.
...
5.  (Optional) Configure a `Timestamp Field` for incremental syncing:
    -   With a timestamp: syncs run every **15 minutes** and only import new/changed records.
    -   Without a timestamp: Clay reads all records every **12 hours** and detects new or changed rows by hash comparison.
6.  Map your Databricks columns to Audience fields.
7.  Review and click `Confirm` — Clay begins importing immediately.
...
-   **Incremental sync:** Runs every **15 minutes** when a `Timestamp Field` is configured, importing only records that are new or changed since the last sync. Without a timestamp field, Clay reads all records every **12 hours** and detects new or changed rows by hash comparison.
-   **Full sync (every 7 days):** Re-reads all records and reconciles deleted records — catching anything the incremental sync may have missed.
