...
-   **All fields must have an exact match.** If you filter by both `Email` and a secondary identifier field (such as a profile URL), a record must match on both to be returned. A record that matches only one field is excluded from the results.
-   **Empty (null) field values never match.** If a record in your Audience has an empty value for one of your filter fields, it will not be returned — even if the other filter fields match. To avoid missing records, ensure the fields you filter on are populated for the records you expect to find.
