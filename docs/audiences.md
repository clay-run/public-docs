...
The fastest way is directly from the **Data Hub**:

1.  In the left sidebar, click **Data Hub**.
2.  Select the **Fields** tab (it opens by default).
3.  Click **+ Add field**, name the field, select a field type (text, email, URL, date, boolean, or number), and save.

Once created, the field is immediately available as a filter in any segment and as a target for `Update Audiences Record` or `Upsert Audiences Record` from any Clay table or workflow enrichment node.

You can also create a new field inline while setting up an `Update Audiences Record` action:
