...
-   In any Clay table, click `Add enrichment` and search for:
    ...
    -   `Lookup in Audiences` pulls data from your Audience into a table row. Use it to reference enriched or signal data in a table workflow without making Salesforce API calls. By default, signal data is returned for the past **90 days** and the action returns a maximum of **5 signal results** per record — adjust the **Signal data to include (days)** setting in the column settings to retrieve older signals (up to 180 days), or use `Get Audiences Activity` when you need more than 5 results.
