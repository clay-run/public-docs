...
Clay's incremental sync picks up Salesforce changes via `SystemModstamp`. If a field value changed in Salesforce but hasn't appeared in Clay yet, here are the most common causes:

**Formula or calculated fields:** Salesforce formula fields do not update `SystemModstamp` when they recalculate — Clay's incremental sync will not pick them up. These fields only appear in Clay after the next weekly full sync. There is no way to force an earlier update for formula fields short of contacting Clay support to trigger an early full sync.

**The field wasn't included in your original import mapping:** Only fields added to the Salesforce import field mapping are synced. If the field is missing from the mapping, add it in your Salesforce source settings — see [A Salesforce field isn't appearing in my audience filters — how do I add it?](#a-salesforce-field-isnt-appearing-in-my-audience-filters--how-do-i-add-it) above.

**The record was modified before the field was added to the mapping:** Even after adding a field to the mapping, only records modified after the change will be re-synced with the new field. Records not touched in Salesforce since then won't update until the weekly full sync.

**The incremental sync hasn't run yet:** On Enterprise, the incremental sync runs every 15 minutes. On Growth, it runs once daily. Changes from Salesforce appear in Clay after the next scheduled sync following the modification.

### I enriched data in my Audience. Why hasn't it appeared in Salesforce yet?

Enriched Audience data writes back to Salesforce on a **separate 24-hour export cycle** — not on the same schedule as the import sync. After an enrichment completes, the updated values appear in Salesforce at your workspace's next scheduled export time (within 24 hours).

If you need data in Salesforce sooner, add a **Salesforce Update Record** action column directly inside the bulk enrichment table — see [How do I write enriched fields back to existing Salesforce records from a bulk enrichment?](#how-do-i-write-enriched-fields-back-to-existing-salesforce-records-from-a-bulk-enrichment) above.

### Can the Audiences export sync write data back to Salesforce Lead records?

No. The scheduled export sync (the 24-hour automated write-back) does not apply to Salesforce Lead records — Lead field mappings in Audiences do not include a Scheduled export rule column, and the export toggle has no effect on Leads.

To write enriched data from Audiences back to existing Salesforce Lead records, add a **Salesforce Update Record** action column directly inside a bulk enrichment table:

1.  Navigate to a segment and click **Enrich** → **Add bulk enrich**.
2.  Add your enrichment columns as usual.
3.  Add **Salesforce Update Record** and set **Record ID** to the Lead's Salesforce ID field (the `00Q…` ID imported from your Salesforce Lead source).
...
-   Active records in **All People** and **All Companies**
-   Records in any named segment (they are subsets of All People / All Companies and not counted separately)
...
-   Activities (Salesforce Tasks and Events) and Opportunities
...
### Why is an audience count different from what I see in Salesforce?

There are several reasons why your Audience record count in Clay may not match the count in Salesforce:

-   **Some records haven't synced yet** — Depending on your plan, incremental syncs run every 15 minutes (Enterprise) or once daily (Growth). Records created or modified in Salesforce since the last sync haven't appeared in Clay yet.
-   **Entity resolution merged records** — If two Salesforce records (for example, a Lead and a Contact) share a matching identifier (email, profile URL, or `ConvertedContactId`), Audiences merges them into a single Clay person record. The same contact appearing as both a Salesforce Lead and a Salesforce Contact counts as two records in Salesforce but one in Clay.
-   **Hard-deleted Salesforce records** — Records permanently deleted in Salesforce are not visible to the incremental sync. They remain in Audiences with **Deleted in source** status until the next weekly full sync marks them, after which they may still appear in the All People view unless archived.
-   **Your import is filtered by SOQL** — If you're using a record subset import, only records that match your SOQL query are imported. Records outside the filter exist in Salesforce but not in Clay.
-   **Salesforce has duplicate records** — Clay's entity resolution may have merged Salesforce duplicates into single Audience records, reducing the count compared to Salesforce.

### How do I exclude a Salesforce field from being written back to Salesforce?

Set the field's write rule to **Never write** in your Salesforce source settings. **Never write** is the default for all newly added field mappings, so no action is required for fields you haven't explicitly configured.

For fields already set to **Always write** or **Write if empty** that you want to stop exporting:

1.  Go to **Settings** → **Sources / Destinations** and click your Salesforce connection.
2.  Select the relevant object tab (for example, **Contacts**).
3.  Find the field and click the **pencil (edit) icon** next to it.
4.  Change the write rule to **Never write**.
5.  Click **Save and review** → **Confirm**.

Clay will no longer update that field in Salesforce on future export cycles.
