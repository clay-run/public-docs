...
3.  Click `Continue` → `Save to People/Companies`.
    ...
    -   If the draft shows a banner that says **"X records from this search already exist in All People,"** those records are already excluded from the merge. Clicking **All people** in step 5 will only add net-new contacts — the existing records are not duplicated.
