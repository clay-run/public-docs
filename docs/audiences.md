...
5.  (Optional) Configure a `Timestamp Field` for incremental syncing:
    -   With a timestamp: Clay only imports new or changed records. Syncs run every **15 minutes** on Enterprise plans, and once every **24 hours** on other plans.
...
-   **Incremental sync:** Runs every **15 minutes** on Enterprise plans (or once every **24 hours** on other plans) when a `Timestamp Field` is configured (for example, `updatedAt`), importing only records that are new or changed since the last sync. Without a timestamp field, the full SQL query reruns every **12 hours** on Enterprise plans (or once every **24 hours** on other plans).
...
-   **Incremental sync:** Runs every **15 minutes** on Enterprise plans (or once every **24 hours** on other plans) when a `Timestamp Field` is configured, importing only records that are new or changed since the last sync. Without a timestamp field, the full SQL query reruns every **12 hours** on Enterprise plans (or once every **24 hours** on other plans).
-   **Full sync (every 7 days):** Re-reads all records and reconciles deleted records — catching anything the incremental sync may have missed.
...
**Note:** Sync cadence is configurable per workspace. If you need warehouse data to refresh more or less often than the schedule above, contact your Growth Strategist.
...
| 3 | Salesforce (Lead) |
| 4 | Snowflake, BigQuery, Databricks |
| 5 | CSV |
...
4.  Open `Field Mapping` and map each column you want to save back to Audiences:
    -   Enable the **Auto-enrich new records** toggle so that any new record entering this segment is automatically passed through the enrichment — typically within 15 minutes.
...
The **Reporting** tab in Audiences shows how Clay has contributed to your audience data and what that data drove downstream. Switch between **People** and **Companies** to scope the entire tab to that record type.

Two summary cards appear at the top — total record count for the selected type, and the Sources connected to it. Below them, four sections break down Clay's impact:

-   **Clay coverage** — how many records Clay sourced, enriched, and actioned, plus a Total records count for unique records covered by any of the three. Set the window to Last 7 days, Last 30 days, or Last 90 days, and switch between Cumulative and Daily views.
...
Enrichments configured with **Auto-enrich new records** enabled automatically process new records entering a segment, typically within 15 minutes. No manual runs are required after initial setup.
...
This approach supports batching and works for both contacts and companies. To automatically push data for new records entering the segment going forward, enable the **Auto-enrich new records** toggle on the bulk enrichment.
...
- **Different source types:** The source type with higher priority wins, regardless of write time. The default priority order, from highest to lowest:
  ...
  4. **Snowflake**, **BigQuery**, and **Databricks**
