...
3.  Select `Criteria` and then add a `Filter` or `Filter group`.
    ...
    -   **Filter groups** combine multiple conditions using their own AND/OR logic. Within a single filter group, all conditions share the same operator — all AND or all OR; changing the operator in the group header switches all conditions at once. Filter groups support **one level of depth only** — you can add individual filters inside a group, but you cannot add a filter group inside another filter group. To mix AND and OR within a segment, add a filter group alongside your individual filters. For example, to build `A AND B AND (C OR D)`: add A and B as top-level filters, click **`+ Filter group`**, then add C and D inside the group. Once the group contains two or more conditions, a small **`and`** button appears between them — click it to switch to **`or`**.

**OR branches are independent.** The top-level connector between filter groups (or between a filter group and an individual top-level filter) can be switched from **and** to **or** — but this changes the mode for the entire top level at once, so all items at the top level are connected by the same operator. When the top level is set to **or**, each item is an independent branch: conditions inside one branch do not apply to any other branch. To build a segment with multiple OR branches that share a common condition — for example, `(Date filter AND Qualification AND Segment = SMB) OR (Date filter AND Qualification AND Segment = Mid-Market)` — create a separate filter group for each branch, include all required conditions (including shared ones) inside each group, then set the top-level connector to **or**.

If your logic would require a filter group inside an OR branch that itself has mixed AND/OR (for example, the branch `Segment = SMB AND (Employees > 200 OR Criteria X OR Criteria Y)`), flatten it into separate OR branches instead — `(SMB AND Employees > 200) OR (SMB AND Criteria X) OR (SMB AND Criteria Y)` — since filter groups cannot be nested. These are logically equivalent.
