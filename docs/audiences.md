...
**Note:** **`does not contain`** and **`does not contain any of`** match records where the field contains a value that doesn't include the specified text — records where the field has no value at all are excluded. For example, filtering `Deal Status → does not contain → Active` to find companies without an active deal will also exclude companies with no deal status set. To include blank-field records, add an `is empty` condition with OR logic: click **`+ Filter group`**, add both `Deal Status → is empty` and `Deal Status → does not contain → Active`, then click the `and` connector between them to switch it to `or`. If a boolean field covers the same case (for example, `Active Deal Pipeline → is false`), use that field directly for a simpler filter.
