...
description: Answers to common questions and troubleshooting steps for Clay's MCP integration, covering connection errors, function issues, ChatGPT-specific problems, credits, data access, and security.
last_synced: 2026-08-06T02:11:27.324Z
---
...
#### **The consent screen warns `Unverified application`**

Clients that register themselves at connection time — a CLI connection like `claude mcp add`, or an editor such as Cursor or VS Code — aren't pre-verified by Clay, so the OAuth consent screen labels them `Unverified application`. This is expected and doesn't block the connection.

**Fix:** None needed if you started the connection yourself — approve it and continue. If the client name is unfamiliar, deny it: a self-registering client supplies its own name, so the name alone doesn't confirm which app is asking. Connecting through an official connector (`Claude → Settings → Connectors`, or [**claude.com/connectors/clay**](https://claude.com/connectors/clay)) skips the warning.
...
**Can I connect Clay MCP to other AI tools like Cursor or VS Code?**  
Yes. Alongside the Claude, ChatGPT, Microsoft Copilot, and Glean connectors, Clay's MCP server accepts clients that register themselves at connection time over OAuth — which covers editors like Cursor and VS Code, and local clients like Claude Code and Claude Desktop. Since Clay hasn't pre-verified these clients, the consent screen labels them `Unverified application` (see Troubleshooting above). Self-hosting the Clay MCP server itself isn't supported — every client connects to Clay's hosted server.
