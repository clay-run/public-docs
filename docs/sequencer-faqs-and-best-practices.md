...
description: Answers to common questions about Clay's Sequencer covering costs, limits, enrollment behavior, troubleshooting, and best practices for cold email deliverability.
last_synced: 2026-09-01T03:04:49.513Z
---
...
Sequencer is Clay's cold email product: you attach a segment of People from Audiences to a campaign, and Clay sends a sequence of up to four emails to every lead that enters it. This page is the catch-all for questions that span the whole product — costs and limits, behavior you can't change, and what to check when something looks off. It also collects the practices that separate campaigns landing in the inbox from campaigns that don't.
...
### What changed in Sequencer 2.0?

The biggest change is where leads come from. Sequencer 1.0 campaigns sync rows from a Clay table in a workbook; 2.0 campaigns attach a segment of People in [Audiences](https://university.clay.com/docs/audiences) and enroll leads continuously as they enter it.

Because every lead is an Audiences record, campaign activity writes back onto that person — and rolls up to their company — so replies and bounces become data you can segment and act on later.

|  | Sequencer 1.0 | Sequencer 2.0 |
| --- | --- | --- |
...
| Development | Maintenance mode — no new features | Where new features ship |

On the `Campaigns` page, 1.0 campaigns are labeled `Table` and 2.0 campaigns are labeled `People`.

### What happens to my existing Sequencer 1.0 campaigns?

They keep running exactly as they do today, on every plan. Sequencer 1.0 is in maintenance mode — it's supported, but new development is going into 2.0 only.

If you're starting something new, build it as a 2.0 campaign. The [Email sequencer](https://university.clay.com/docs/email-sequencer) doc remains the reference for the table-based flow.

### Which plans include Sequencer 2.0?

Launch, Growth, and Enterprise. Sequencer 1.0 stays available on all plans.
