...
When an action column's response exceeds the 200 kB limit, the cell shows **"Cell data size exceeds limit (200 kB)"**. For action columns that write data to an external system — such as Airtable Create record, HubSpot Create contact, or Salesforce Upsert record — this warning is display-only: the external write completed successfully. Clay stores as much of the API response as fits in the cell preview, but the record was created (or updated) in the destination.
...
-   **Extracting large enrichment outputs to formula or text columns:** Formula and text columns enforce an 8 kB limit. When you use a formula to pull content from a large enrichment — for example, job postings from a **Find Active Job Openings** or **Find Open Jobs** enrichment, or articles from a **Find Most Recent News** enrichment — the cell shows **"Cell data size exceeds limit (8 kB)"** or goes blank when the full enrichment output exceeds 8 kB. Five workarounds:
    ...
    -   **Map the enrichment column directly in a destination action field.** When exporting data to a destination integration (for example, a **Snowflake Insert Row** or **HubSpot Update Record** action), configure that action to read from the enrichment action column directly instead of first pulling the data into a formula column with `JSON.stringify({{Enrichment Column}})`. Because enrichment action columns hold up to 200 kB, the destination action receives the full payload without hitting the 8 kB formula-column restriction.
    -   **Use a Clay Workflow instead of a table.** Clay Workflows don't enforce the same cell size limits as tables — workflow step outputs can hold large payloads without being capped at 8 kB. If you need to process or export large enrichment outputs (for example, syncing the full output of a **Get Audiences Activity** enrichment to Snowflake), a Workflow lets you pass the complete data to downstream steps without truncation.
