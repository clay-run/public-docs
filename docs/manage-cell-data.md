...
-   **HTTP API columns and webhooks:** HTTP API columns can return responses that exceed the 200 kB action column limit, producing the error **"The result of this run exceeded the cell size limit (200 kB)"**. This limit is a hard cap and cannot be increased. To work around it, open your HTTP API column settings, go to the **Configure** tab, and add the JSON field paths you need to the **Response values to return** field — for example, enter `data.user.email` to store only that value instead of the full API response. You can add multiple paths as separate entries. See [Step 6 of the HTTP API guide](http-api-integration-overview.md#step-6-response-values-to-return) for a full example.
