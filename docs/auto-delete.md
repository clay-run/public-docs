...
**Processing large HubSpot lists:** If you have a HubSpot list exceeding 50,000 records (for example, 100k companies) that you want to enrich all at once rather than stream continuously, [Bulk Enrichment](bulk-enrichment.md) is the recommended approach. Export your HubSpot list to CSV, then create a Bulk Enrichment table using the **Import from CSV** source type — it handles large datasets without the 50,000-row limit. Note that HubSpot is not yet available as a direct Bulk Enrichment CRM import source (Salesforce and CSV only); CSV export from HubSpot is the path today.
