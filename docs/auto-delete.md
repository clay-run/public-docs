...
4.  Under **Auto-delete mode**, select one of the following options:
    ...
    -   **Delete when all actions finish** — Deletes rows once all action columns have finished running.
        ...
        -   **How the cleanup sweep is triggered:** The cleanup does not run continuously — it fires each time an enrichment cell is written in the table (by auto-run or a manual column run). When the sweep runs, it checks every row in the table: a row is eligible for deletion when all action columns are in a non-running state (not actively running, queued, or retrying). Cells that produced any result — success, error, or no data found — count as finished. When no Success column is selected, cells whose run condition was not met also count as finished.
        -   **If auto-run is disabled on all action columns:** No enrichment cells are ever written, so the cleanup sweep is never triggered and rows will not be auto-deleted, regardless of how many rows accumulate in the table. To keep auto-delete working, ensure at least one action column has auto-run enabled — or manually run your action columns so that results are written and the sweep fires.
        -   **Success column and run conditions:** When a Success column is selected, that column must produce a successful result for the row to be deleted. A cell skipped because its run condition was not met does not count as successful and will block auto-delete for that row. If your workflow intentionally skips the success column for some rows via a run condition, switch to **Delete based on conditional rules** mode instead — or remove the Success column setting so that rows whose success column was skipped are treated as complete.
...
All other source types — such as CRM integrations (Salesforce, HubSpot), CSV and bulk uploads, Find people sources (including Sales Navigator), Snowflake, and database connections — will continue accumulating toward the 50,000 limit even when auto-delete is enabled. Auto-delete will still delete rows from the table for those sources, but the underlying source record count is not cleared.
...
**Processing more than 50,000 records with an incompatible source:** If your source is incompatible and you need to handle a volume larger than 50,000 records, you have two options:

-   **Switch to a webhook-based pipeline.** Move the data collection outside of Clay and push results in via a Clay webhook source. For example, use an external automation tool (such as Make, Zapier, or n8n) to run your Sales Navigator search and POST each record to a Clay webhook. Because webhook sources are compatible with auto-delete, the pipeline processes records continuously without hitting the 50,000-record limit.
-   **Split across multiple tables.** Create a new table with the same source settings. Each new table starts its source record count at 0 of 50,000, so you can continue processing until that table reaches its limit, then create another. This works well when your searches are naturally segmented (for example, by region or persona) but requires manual management.
