...
**Webhook source without auto-delete warning:** If your table uses a webhook source and auto-delete is disabled, Clay displays a warning banner: "This table is receiving data continuously without auto-delete enabled." The banner advises enabling auto-delete to avoid reaching the table's 50,000 record limit. Webhook sources track a cumulative submission count — the counter increments with every accepted payload and does not reset when rows are deleted from the table. Once the cumulative limit is reached, Clay returns a `403 Record limit reached for webhook` error to the sending system and stops accepting new payloads. Enable auto-delete to bypass the 50,000 submission cap and allow the webhook to accept data without interruption.
