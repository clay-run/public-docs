...
**Webhook source warning (auto-delete disabled):** On Enterprise plan tables where auto-delete is available but not yet enabled, a warning banner appears when the table has a webhook or webhook subscription source and all attached sources are compatible with auto-delete. The banner title reads "This table is receiving data continuously without auto-delete enabled." and the description reads "Enable auto-delete to avoid reaching the table's 50,000 record limit." Once a webhook source reaches 50,000 records, the source stops accepting new records — the sending system receives an error response rather than a 200 status, but no further in-product notification appears inside Clay, so incoming data can be lost without a visible alert. The warning can be dismissed. If any incompatible sources are attached to the same table, the incompatible source banner takes priority and this warning does not appear.
