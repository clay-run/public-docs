...
All other source types — such as CRM integrations, Snowflake, and database connections — will continue accumulating toward the 50,000 limit even when auto-delete is enabled. Auto-delete will still delete rows from the table for those sources, but the underlying source record count is not cleared. This is because these sources maintain a cumulative import history for deduplication — to prevent re-importing records that have already been processed on previous runs. The count reflects every record ever imported by the source, not just the rows currently visible in the table. Deleting rows from the table does not reduce this count, and records that no longer match your source filters or query are still counted in the total.
...
**Incompatible source banner:** If auto-delete is already enabled and your table has one or more incompatible sources, a warning banner appears on the table: "Auto-delete is on with a source that isn't compatible." The banner lists each incompatible source with its accumulated record count (e.g., 12,000 / 50,000) so you can monitor how close each source is to its limit. For incompatible sources, auto-delete will continue removing rows from the table, but the source record count will keep growing with each import run. Once the source reaches 50,000 cumulative records, it will stop importing new data. To continue importing at that point, you will need to delete and re-create the source.
