...
5.  Optionally, enter a value in the **Number of rows to keep** field. This controls how many of the most recent **deletion-eligible** rows are retained — it applies only to rows that meet your deletion criteria (the filter conditions you defined, or rows where all actions have finished), not to all rows in the table. Rows that do not match your deletion criteria are unaffected and are not counted toward this limit. Leave the field empty to use the default of 100 rows. Enter **0** to delete all eligible rows with no retention window. **This value does not block new inserts** — the table continues accepting new rows until it reaches the 50,000-row hard ceiling, regardless of the keep-limit setting.
