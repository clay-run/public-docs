...
**Webhook source warning — auto-delete off (Enterprise plan):** On Enterprise plans, if your table has a webhook source and auto-delete is disabled, Clay shows a warning banner titled "This table is receiving data continuously without auto-delete enabled." with the description "Enable auto-delete to avoid reaching the table's 50,000 record limit." This banner alerts you before incoming webhook records are silently discarded once the 50,000-row limit is reached. To act on the warning, click the auto-delete icon (archive icon) in the bottom toolbar, or click the table title and select **Enable auto-delete** from the dropdown.
