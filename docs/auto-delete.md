...
**Webhook source warning (auto-delete off):** If your table has a webhook or webhook subscription source and auto-delete is not yet enabled, Clay displays a warning banner: "This table is receiving data continuously without auto-delete enabled." The banner also shows: "Enable auto-delete to avoid reaching the table's 50,000 record limit." This warning appears on Enterprise plan workspaces where auto-delete is available but has not been turned on for a table actively receiving webhook data. To resolve the warning, enable auto-delete following the [steps above](#enable-auto-delete). The banner can be dismissed for the current session by clicking the dismiss button.
