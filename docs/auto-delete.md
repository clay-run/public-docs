...
**Webhook source without auto-delete banner (Enterprise plan):** If auto-delete is disabled on a table with a webhook source, Clay displays a warning banner: *"This table is receiving data continuously without auto-delete enabled."* This banner appears only on Enterprise plan workspaces. It prompts you to enable auto-delete before the table reaches the 50,000-record limit and incoming webhook submissions start being rejected with a `403` error. To resolve it, follow the [Enable auto-delete](#enable-auto-delete) steps above.
