...
-   Rows that exist only in the sandbox (not in your production table) will not be copied when you publish.
...
In both cases, Clay also copies your sandbox enrichment results back to the corresponding rows in your production table — so you don't have to re-run those enrichments and spend credits twice.
