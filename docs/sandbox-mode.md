...
When you are ready to publish the changes in your sandbox to your regular table, you have two options. In both cases, Clay automatically copies your sandbox row results to the production table — so any rows you already ran in the sandbox are reflected in production without re-running them:

-   `Publish and don't Run` syncs all your column configuration changes and copies sandbox row results to your production table, but does _not_ start a run for remaining rows. You can manually run them later.
-   `Publish and run` syncs all your column configuration changes, copies sandbox row results to your production table, **and** then runs all remaining rows (those not already computed in the sandbox) on the full table.
