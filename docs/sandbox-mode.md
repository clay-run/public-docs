...
-   When you publish, enriched data from rows you ran in your sandbox is automatically copied to the production table — so you don't need to re-run those rows or spend credits twice.

### Publishing changes

When you are ready to publish the changes in your sandbox to your regular table, you have two options:

-   `Publish and don't Run` will sync all your column configuration changes to all data but will _not start_ a run for any of these columns. Rows you ran in your sandbox will have their enriched data copied to the production table automatically. You would need to manually run any remaining rows later.
-   `Publish and run` will sync all column configuration changes to all data **and** run all affected columns on rows in the full table. Rows you already ran in your sandbox will have their enriched data copied directly — so you won't be charged twice for those rows.
