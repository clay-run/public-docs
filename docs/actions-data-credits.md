...
**To avoid double-charging on overlapping lists:** Merge your lists into a single table first and use the **Deduplicate rows** feature (right-click any column header → **Deduplicate rows**) to remove duplicate entries based on LinkedIn URL or another stable identifier. Enriching the merged, deduplicated table ensures each person is processed and charged only once.
