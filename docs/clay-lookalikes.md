...
**People Lookalikes** is available only as an in-table enrichment — there is no standalone source or table-builder for people. See [`Action` Find People Lookalikes](#action-find-people-lookalikes) below.
...
Find people similar to a seed person. Unlike company lookalikes, this is available only as an in-table enrichment — there is no standalone source or table-builder for people. The enrichment identifies companies similar to the seed person's employer and then finds people with matching titles and seniority levels at those lookalike companies.
