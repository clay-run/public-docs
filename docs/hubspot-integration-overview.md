...
The HubSpot source polls HubSpot on the schedule you configure in Clay — it is not a real-time sync. Records newly added to HubSpot appear in your Clay table only on the next scheduled run. Available frequencies are daily, weekly, and monthly; hourly is available on Enterprise plans only. To view or change the schedule, click the source column title, select your source, and check the **Run settings** section.
...
HubSpot Activities — including calls, emails, meetings, notes, and tasks — are not available as importable object types through Clay's HubSpot integration. Clay's HubSpot integration connects to HubSpot's CRM Objects API, which covers **Company**, **Contact**, **Deal**, **Lead**, and **custom objects**. It does not connect to HubSpot's separate Engagements API where Activity types live — so Activity types do not appear in the object type selector regardless of your HubSpot plan, settings, or permissions.
