...
**Note:** When the object type is **Lead**, the Create Object step also surfaces inline association fields, letting you associate the new Lead with a Company and/or Contact directly within this action. For all other object types (for example, linking a contact to a company), associating a created record is not part of the Create Object step — it requires a separate **Create association** action. See [How do I create a contact-to-company association in HubSpot from Clay?](#how-do-i-create-a-contact-to-company-association-in-hubspot-from-clay) for a step-by-step workflow.
