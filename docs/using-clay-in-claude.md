...
If your workspace has Clay Audiences enabled (generally available on Launch, Growth, and Enterprise plans), you can use natural language to filter and look up accounts — for example, "Show me my open-pipeline accounts in the northeast" or "Tell me about [Account]." By default, results include all accounts in the workspace. If your admin has enabled **Restrict account querying by Salesforce owner** in [MCP settings](https://university.clay.com/docs/mcp-settings), results are scoped to accounts you own in Salesforce. If you see an error that contact queries aren't available, ask your admin to disable `Restrict account querying by Salesforce owner` on the `MCP` page in the workspace sidebar — when this toggle is on and Salesforce is connected, contact queries return an error. Note that the MCP page is only visible to workspace admins. If the restriction toggle is on but queries return no results, ensure `Sync user IDs from audiences` is also on and your Salesforce account ownership is correctly mapped.
