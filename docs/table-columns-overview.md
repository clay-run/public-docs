...
-   Deleted rows do not go to Trash. A brief **Undo** alert appears immediately after the dedupe runs — click it to restore rows before the alert closes. After that, the Clay support team can restore deleted rows within **30 days** of deletion (subject to your table's 50,000-row limit). See [Recover deleted rows](delete-content-within-your-workspace.md#recover-deleted-rows) for details.
