...
The error message includes the ID of the affected table. If that table ID does not match the table you are currently editing, the enrichment limit has been reached on a different table in your workbook — for example, a company table or reference table that your current table reads from via a Lookup Rows or Company Lookup action. Open your workbook, navigate to the table shown in the error message, and resolve the enrichment column count there (delete or consolidate columns on that table, not the one you started from). If you cannot identify the table by its ID, contact support and share the table ID from the error message.
...
-   **Remove unused providers from a waterfall.** Each provider step in a waterfall counts as a separate enrichment column toward the limit. For email and mobile phone waterfalls that have validation enabled, each provider also adds a validation column — so a 5-provider email waterfall uses up to 10 enrichment columns (5 provider columns + 5 validation columns). To reduce the count without deleting the entire waterfall: click the waterfall column header → **Edit Column** → **Full Configuration** → delete any provider steps you no longer need. Deleting a provider removes its enrichment column and, if applicable, its paired validation column. You can also turn off the validation setting in Full Configuration to save one column per provider without removing any providers.
