...
**If your workspace uses SSO (such as Okta or another SAML identity provider):** Clay's SSO creates a Clay account automatically the first time a user signs in — SSO handles authentication and account creation, but does not automatically add users to your enterprise workspace. To land in your organization's workspace, a user must have accepted a Clay workspace invite before their first SSO login. If a user signs in via SSO without first accepting an invite, they are routed to a new, empty personal workspace instead of your organization's workspace. Sending a Clay invite after the fact still works — once the user accepts the invite and logs in via SSO, they will be directed to the correct workspace. For Enterprise customers, SCIM Directory Sync (currently in beta) automates workspace provisioning — contact Clay support or your Growth Strategist to enable it.
...
For automated user lifecycle management through an identity provider (such as automatically adding or removing users when accounts are created or deactivated in Okta), see [Single Sign-On (SSO)](/docs/single-sign-on) — which covers SSO authentication behavior and SCIM Directory Sync (in beta for Enterprise customers) for automated workspace provisioning and deprovisioning.
