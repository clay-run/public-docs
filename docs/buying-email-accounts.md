...
From the `Campaigns` homepage, go to the `Email Accounts` tab and click `Add Email Accounts`. Select `Buy managed accounts` to open the purchase flow.
...
No. The **Buy managed accounts** flow can only provision inboxes on new domains purchased through Clay — it cannot attach inboxes to a domain you already own.
...
To scale your campaign sending capacity, buy additional email accounts directly in Clay using the **Buy managed accounts** option described above, or connect additional mailboxes via Google OAuth, Microsoft Outlook OAuth, or SMTP. Each connected account has its own independent daily sending budget and sender reputation. See the [Email sequencer guide](email-sequencer.md) for connection options.
