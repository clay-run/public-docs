...
> **Removed:** Ocean.io's lookalike enrichments and sources have been removed from Clay. [Clay Lookalikes](clay-lookalikes.md) is the recommended replacement — it covers company and people lookalikes natively, with clustering, Sculptor support, and audience-scale inputs. If you see the error **"This action is deprecated and is no longer supported"** on an existing Ocean.io column, the integration has been removed — migrate to Clay Lookalikes to restore this functionality.
