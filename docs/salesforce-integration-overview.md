...
**Scheduled refresh:** The list source supports automatic scheduled runs on **paid plans** (Trial and above; not available on the Free plan). After setting up your source, change **Run this source** to **On a schedule** to have Clay automatically pull in new records without manual re-runs. For setup steps, see [Scheduled sources](scheduled-sources.md).
...
**Scheduled refresh:** The report source supports automatic scheduled runs on **paid plans** (Trial and above; not available on the Free plan). After setting up your source, change **Run this source** to **On a schedule** to have Clay re-run the report automatically and pull in new records on your chosen cadence. For setup steps, see [Scheduled sources](scheduled-sources.md).
...
Look up records in Salesforce using a custom SOQL query. Use this when the standard **Lookup record** action returns too many matches, when you need to filter on multiple fields at once (e.g., website AND country code), or when the Lookup record action returns **"Error: Bad Request"** (which can occur due to Salesforce API limits on object schemas with many cross-object field references — see [Salesforce's documentation](https://developer.salesforce.com/docs/atlas.en-us.salesforce_app_limits_cheatsheet.meta/salesforce_app_limits_cheatsheet/) for details).
