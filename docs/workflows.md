---
title: Workflows
description: How to build, configure, and run Workflows — Clay's graph-based orchestration layer that processes one record at a time through triggers, enrichment steps, conditionals, Claygent agents, and code.
last_synced: 2026-08-11T15:41:37.734Z
---
...
Workflows is Clay's orchestration layer. A workflow is a graph of connected nodes that a trigger starts, and it runs once per record or event — where a table enriches a list row by row, a workflow moves a single record along a path, with branching, conditions, and logic on the way.
...
| --- | --- | --- | --- |
| Run enrichment | Runs one Clay action, like finding an email, enriching a company, or writing to a CRM | Any single data lookup or external action | 1 action |
| Run function | Calls a reusable Clay function | Reusing governed enrichment or qualification logic | Charges the actions inside it |
| Run Claygent | Runs a Claygent — an agent with a prompt, a model, and its own tools | Work that needs reasoning or judgment | Claygent credits |
| Update audience members | Writes results back to a contact or account in Audiences | Persisting what the run produced | Free |
| Conditional | Branches the graph on rules, code, or an AI judgment | Sending a record down one of several paths | Free for Rules and Code; about 1 action for AI |
...
**Reading how a decision was made.** A trace doesn't just tell you a step ran — it shows the logic that step applied. Opening a run lights up the path it actually took on the graph, so you can see which branch each record went down and which steps never fired.
...
That's what you work backwards from when a record ends up somewhere you didn't expect — you can see the decision itself, not just its outcome, which is the part a table never showed you.
...
Workflows bills on the same two meters as the rest of Clay. **Data credits** cover enrichment and AI usage, and **actions** count one per action, per record, per run, exactly as in a table.

Workflows is available on Enterprise, Growth, and Launch plans indefinitely, and on legacy Starter, Explorer, and Pro plans through the end of 2026. Free and trial plans include it as well, with some steps unavailable.

The orchestration itself is free. Triggers, conditionals, `Run code` steps, routing, and writing back to Audiences cost nothing, so you're only paying for lookups and AI. That makes gating the main cost lever you have: put a cheap `Rules` or `Code` conditional early, and records that don't qualify stop at zero credits while your spend goes to the ones worth pursuing.
...
**Scale.** Workflows is built for individual records, comfortably up into the hundreds of thousands. Past roughly a million records in one batch, [Bulk enrichment](https://university.clay.com/docs/bulk-enrichment) is the better fit.

Graph size isn't capped. A table limits how many columns you can add; a workflow doesn't limit steps, so a long play can live in one graph instead of being split across several tables.
...
Have questions about Workflows? See [Workflows FAQs](https://app.notion.com/p/Ads-on-Documentation-university-clay-com-docs-clay-ads-3a57e66eb014813c9701e66f565f2d36?pvs=21).
