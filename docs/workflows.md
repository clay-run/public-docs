---
title: Workflows
description: Covers how to build, trigger, and observe Workflows — Clay's graph-based orchestration layer for automating record-level plays with branching, conditions, and actions.
last_synced: 2026-09-08T17:53:08.015Z
---
...
| Delay | Pauses the branch it sits on before the next node runs | Giving a downstream step time it needs | Free |
...
A `Delay` node pauses the branch it sits on before the next node runs. Use it when a downstream step needs time that isn't its own — letting an export land before you read the result back, or spacing out writes to an external system that rate-limits you.

To add one, open the node palette and choose `Delay`, then set two fields in its `Configuration` panel:

1.  `Duration` — how long to wait, as a whole number of at least 1.
2.  `Unit` — either `Seconds` or `Minutes`.

The longest single delay is 60 minutes, and the delay applies every time the branch runs
...
Workflows is available on the Enterprise, Growth, and Launch plans indefinitely, and free and trial plans include it as well, with some limitations.

Legacy plans differ by tier. Legacy Enterprise customers keep Workflows indefinitely, with no change to their access. The legacy self-serve plans — Starter, Explorer, and Pro — have access for a limited time, after which Workflows moves to a free-plan experience rather than switching off. See [Workflows FAQs](https://university.clay.com/docs/workflow-faqs) for what changes then.
