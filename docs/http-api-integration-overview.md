...
**Note:** Intentionally empty objects (`{}`) are **not** removed by this toggle — they are treated as valid values and preserved in the outgoing request. Only `null`, `undefined`, and empty string values are stripped. This matters for APIs that use empty objects as tool configurations, for example `"urlContext": {}` in Gemini API requests.
