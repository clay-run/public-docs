...
description: Import prospects, accounts, and email activity from Nooks into Clay, and enroll prospects in Nooks sequences.
last_synced: 2026-09-23T19:04:23.942Z
---
...
Each Nooks action costs 1 credit per run. `Get prospect`, `Get account`, `Get user`, and `Get sequence state` refund that credit when the lookup finds no match, so unmatched rows don't cost you anything.
