...
description: Import Nooks prospects, accounts, and emails into Clay, and enroll prospects in Nooks sequences from Clay.
last_synced: 2026-09-24T19:42:34.032Z
---
...
**Note:** The Nooks integration is available on Launch, Growth, and Enterprise plans, and on legacy Explorer and Pro plans.
...
-   **Email:** The prospect's primary email address.
-   **Professional profile URL:** The prospect's professional profile URL.
...
-   **Professional profile URL:** The company's professional profile URL.
...
-   **Opened at:** When the email was first opened.
-   **Open count:** How many times the email was opened.
-   **Replied at:** When the recipient replied.
-   **Prospect ID:** The Nooks ID of the prospect the email was sent to.
...
**Note:** Sequences can't be created from Clay, so build them in Nooks first.
...
Look up a Nooks user by email or CRM user ID and return their Nooks user ID and details. Most useful for filling the `Owner` input on `Enroll prospect in sequence` per row, since that input needs a Nooks user ID rather than an email address.

**Inputs**

Provide at least one identifier:

-   **Email:** The email of the Nooks user to look up.
-   **CRM user ID:** The ID of the user in your connected CRM, such as a Salesforce user ID or owner ID.

If you fill in both fields, the user has to match both to be returned. If more than one Nooks user matches, the action returns an error listing the matches so you can narrow the lookup.

**Outputs**

-   **User ID:** The user's Nooks ID.
-   **Full name:** The user's full name.
-   **Email:** The user's email address.
-   **CRM user ID:** The user's ID in your connected CRM.
-   **Created at:** When the user was created in Nooks.
-   **Updated at:** When the user was last updated in Nooks.
...
-   **Sequence:** The sequence to enroll the prospect in. Defaults to a dropdown of the enabled sequences in your Nooks workspace.

This can also be switched to `Text with tokens` to map a column. The field expects a Nooks sequence ID, such as `1effa677-12c1-49d2-a0b1-5affdc65297d`.

-   **Owner:** The Nooks user who will own this enrollment — outreach is sent on this user's behalf. Defaults to a dropdown of the users in your Nooks workspace, listed by full name with their email in parentheses.

If you keep owner data in Clay or your CRM, click the gear icon beside the input, choose `Text with tokens`, and map a column instead. The field expects a Nooks owner ID, such as `cIypHdIN42Mbh08iZ8gK1IvIe7q1` — an email address or a name won't resolve. You can get that value from the `User ID` output of the `Get user` action.

Optional:

-   **Mailbox:** The mailbox to send email outreach from. Displays as a dropdown of mailbox email addresses, filtered to the mailboxes belonging to the `Owner` you selected. Defaults to the owner's primary mailbox if left blank.
...
Re-running this on an enrollment that's already finished won't error, so it's safe to run across the same column more than once.
...
On current pricing plans, each Nooks action uses 1 action per run and no data credits. `Get prospect`, `Get account`, `Get user`, and `Get sequence state` refund that action when the lookup finds no match, so unmatched rows don't cost you anything. Workspaces on earlier pricing plans don't spend credits on Nooks actions.
