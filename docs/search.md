---
title: Search (Beta)
description: Clay Search lets you find companies, people, and jobs in a single query across Clay's data network, with results saveable as a one-time list or an always-on Audience.
last_synced: 2026-07-28T15:42:58.118Z
---
...
**Note:** Search is in open beta ahead of general availability. It's on by default for eligible workspaces, with the option to opt out from the banner. A few capabilities are still rolling out (see the FAQs), and how enrichments are returned and priced inside a search may change before GA.
