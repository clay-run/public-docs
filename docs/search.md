---
title: Search (Beta)
description: How to use Clay's Search (Beta) to find companies, people, and jobs in a single query and save results as one-time lists or always-on Audiences.
last_synced: 2026-09-22T04:59:29.928Z
---
...
### Can I filter by a company's place in a corporate hierarchy?

Yes. Add the `Hierarchy status` filter to a Companies search and set it to `Ultimate parent` to return only top-level parents, or `Subsidiary` to return only companies that sit beneath one. You can select both.

This means you can carve a list down to just the parent entities you sell into, without enriching every row first to work out how the companies are related.
