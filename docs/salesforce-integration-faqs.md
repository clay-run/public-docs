...
If you need to retrieve more than 5 records — for example, all contacts on an account or all members of a Salesforce campaign — use the **Lookup Records via SOQL** action instead. With SOQL you control how many results are returned via your own `LIMIT` clause, or omit it to return all matches.
...
**All members of a Salesforce campaign:**

```sql
SELECT Id, CampaignId, ContactId, LeadId, Status, FirstName, LastName, Email, Phone, CreatedDate
FROM CampaignMember
WHERE CampaignId = '/Campaign ID'
```

Replace the column placeholders with the relevant columns from your Clay table using the `/` picker in the query editor. You can use an AI assistant to help write the query — for example: "write a SOQL query to return all members for a given Salesforce campaign ID." For SOQL syntax reference, see [Salesforce SOQL](salesforce-soql.md).
