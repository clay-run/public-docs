...
## Why am I seeing a `DUPLICATES_DETECTED` error on my Salesforce Create Record action?

This error is returned by Salesforce, not generated by Clay. It means your Salesforce org has a duplicate rule that detected an existing record matching the one Clay is trying to create.

There are three ways to handle it:

1.  **Update the existing record instead of creating a new one.** Add a **Lookup record** (or **Lookup records via SOQL**) column before your **Create record** column to find the existing record and retrieve its ID. Then replace the **Create record** action with an **Update record** action using that ID. This is the recommended approach when a matching record already exists in Salesforce and should be updated rather than duplicated.

2.  **Enable the Duplicate Rule Override.** In your **Create record** column settings, turn on **Duplicate Rule Override**. With this enabled, Clay sends an `allowSave=true` header to Salesforce that bypasses its duplicate check, so the record is created even if a duplicate exists. Use this only when you intentionally want to create the record regardless of any existing match.

3.  **Adjust your Salesforce duplicate rule.** In Salesforce Setup, you can modify the duplicate rule to allow saves, narrow its matching criteria, or exclude records created via the API. See [Salesforce duplicate rules documentation](https://help.salesforce.com/s/articleView?id=sales.duplicate_rules_map_of_reference.htm&type=5) for details.
