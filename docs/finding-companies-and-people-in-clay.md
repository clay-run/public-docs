...
1. In your company table, add a **Lookup Multiple Rows in Other Table** column. Set `Table to search` to your people table, `Target column` to the company LinkedIn URL column in the people table, `Filter operator` to `Equals`, and `Row value` to the company LinkedIn URL column in this table. This gives each company row a live count of how many people have already been found for it.
