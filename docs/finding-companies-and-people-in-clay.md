...
-   **Find People at These Companies** — creates a separate people table with one row per contact, linked back to the company. Best when you want to run enrichments on each contact individually (work email, phone, etc.) or need to rank/filter contacts before saving. **To access:** In your company table, click **Tools** (top right) → **Sources** tab (labeled **Import** in some accounts) → **Find People at These Companies**. The setup panel includes full filters for job title, seniority, location, experience, and more — set your criteria, preview the results, then click **Continue** → **Save to People** (or **Save** in the standard wizard path) to create the people table. The setup panel also includes a **Limit per company** field to cap the number of contacts returned per company, and a global **Limit results** setting to cap the total across all companies. On large company lists, the in-table **Find People at These Companies** enrichment action gives you the most predictable per-company control, since it processes each company row individually.
...
**If repeated runs continue to return 0 new rows and your table is well below its limits** (no "source has exceeded your plan's limit" error is shown), the source has already found all available people matching your current criteria — there are no new contacts in the dataset that fit your filters. To source more contacts, widen your search: add more job title keywords, include additional industries or geographies, or expand your company size range.
