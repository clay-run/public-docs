...
-   **Find People at These Companies** — creates a separate people table with one row per contact, linked back to the company. Best when you want to run enrichments on each contact individually (work email, phone, etc.) or need to rank/filter contacts before saving. **To access:** In your company table, click **Tools** (top right) → **Sources** tab (labeled **Import** in some accounts) → **Find People at These Companies**. The setup panel includes full filters for job title, seniority, location, experience, and more — set your criteria, preview the results, then click **Continue** → **Save to People** (or **Save** in the standard wizard path) to create the people table. The setup panel also includes a **Limit per company** field intended to cap the number of contacts returned per company, but this field is currently broken in production — results may exceed or fall short of the value you set. Until the fix ships, do not rely on **Limit per company** to enforce a per-company cap; use the global **Limit results** setting instead, or switch to the in-table enrichment action which processes each company row individually.
...
**Known issue (as of June 2026):** The **Limit per company** field is currently unreliable in production — the cap is not consistently enforced and actual results may exceed or fall short of the value you set. Engineering is actively investigating. Until a fix ships, treat any per-company cap as approximate.
