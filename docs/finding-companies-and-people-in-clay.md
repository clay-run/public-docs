...
Re-running the source is all that's needed. It searches across all companies in the linked table — including any you just added — and appends new people while deduplicating against rows already in the table. To re-run: click the source column header in the people table and select **Run**. If your source is already configured to run **On a schedule**, no manual action is needed — the next scheduled run automatically searches across all companies currently in the linked table, including any added since the previous run. Only net-new contacts are added as rows; contacts already in your people table are skipped automatically. To run immediately rather than wait for the next scheduled run, click **Run now** in the source's Run settings panel.
...
**Note:** Column deduplication removes entire rows from your table based on matching values. There is no built-in undo — export your table to CSV before running a dedupe if you want a backup you can re-import. For full rules and recovery options, see [Dedupe columns](table-columns-overview.md).
