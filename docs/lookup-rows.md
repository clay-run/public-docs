...
-   **Avoid Number-type columns as lookup keys — convert to text first**: A **Number**-type `Target column` causes a type mismatch with text `Row value` inputs. If the Row value is a non-numeric text string, the lookup produces the error *"We were unable to search for a row. This may be because the value you are searching for has an unexpected type. Please check your inputs and try again."* If the Row value is a numeric-looking string (e.g., `"12345"`), the lookup runs without error but returns "No Record Found" — because the number `12345` and the text `"12345"` are not equal. **Workaround:** In both tables, add a formula column that converts the number to a text string — for example, `String({{Clay Company Id}})` — then configure your lookup to match on those text columns instead of the number columns directly.
...
-   **Avoid Number-type columns as lookup keys — convert to text first**: A **Number**-type `Target column` causes a type mismatch with text `Row value` inputs. If the Row value is a non-numeric text string, the lookup produces the error *"We were unable to search for a row. This may be because the value you are searching for has an unexpected type. Please check your inputs and try again."* If the Row value is a numeric-looking string (e.g., `"12345"`), the lookup runs without error but returns no matches — because the number `12345` and the text `"12345"` are not equal. **Workaround:** In both tables, add a formula column that converts the number to a text string — for example, `String({{Clay Company Id}})` — then match on those text columns instead.
