...
-   **Cross-row ranking and group comparisons** — formula columns evaluate one row at a time and have no awareness of sibling rows, so COUNTIF-style logic or rank-within-group comparisons (e.g., "what is this row's rank among all rows sharing the same category?", "which row in this group has the highest score?") cannot be done in a formula alone. To rank rows within a group or identify the top scorer: add a self-lookup that matches on the shared group column (so each row gets back every other row in its group), then add a formula column that reads the returned `records` array and computes the rank or compares score values. Keep in mind that a self-lookup always includes the current row in its results — account for that when reading counts or positions.
