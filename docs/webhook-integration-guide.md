...
**Submission limit:** Each webhook source accepts up to 50,000 submissions. This is a cumulative lifetime count — every accepted submission increments the counter, and deleting rows from the table does **not** reduce it. Once you reach this limit, Clay returns a `403 Record limit reached for webhook` error and new submissions are rejected. To resume receiving data without rebuilding your workflow, you have two options: **(a) Add a new webhook source** to the same table — this starts a fresh 50,000 counter with a new URL, no table duplication required. **(b) Enable [auto-delete](https://www.clay.com/university/guide/auto-delete)** (Enterprise plan) on your current table — when auto-delete removes rows, it also reduces the source record count, so the same webhook URL can keep accepting data indefinitely.
...
4. **Submission limit reached** — See the [Limits](#limits) section. Once a webhook source hits 50,000 submissions, Clay returns a `403 Record limit reached for webhook` error and stops creating rows. This limit is cumulative — it counts all submissions since the webhook was created, and manually deleting rows from the table does not reset it. To resume receiving data without rebuilding your table, you have two options: **(a) Add a new webhook source** to the same table — this starts a fresh 50,000 counter and gives you a new URL to update in your external system, no table duplication required. **(b) Enable [auto-delete](https://www.clay.com/university/guide/auto-delete)** (Enterprise plan) — when auto-delete removes rows, it also reduces the source record count, so the same webhook URL can keep accepting data indefinitely.
