...
**Enterprise Plan — run a webhook indefinitely:** Enable [auto-delete](https://www.clay.com/university/guide/auto-delete) (also called passthrough tables) on the table that receives your webhook data. When passthrough mode is active, the webhook source takes a separate code path that **bypasses the 50,000 submission limit entirely** — a single webhook URL can keep accepting data indefinitely without ever hitting the cap. This is the recommended approach for automated enrichment pipelines. Auto-delete is available on Enterprise plans and works for webhook, send-table-data, signal, and Audiences sources.

Auto-delete is a **table-level setting** — you enable it on the table that directly receives the webhook data, not on the webhook source itself. To open the settings, click the **archive icon** in the bottom toolbar of the table, or click the **table title** and select **Enable auto-delete** from the dropdown. You only need to enable auto-delete on the intake table (the first table in your pipeline that receives data from the webhook) — downstream tables do not need it enabled to unblock the webhook. See the [auto-delete guide](https://www.clay.com/university/guide/auto-delete) for full setup steps.
...
4. **Submission limit reached** — See the [Limits](#limits) section. Once a webhook source hits 50,000 submissions, Clay returns a `403 Record limit reached for webhook` error and stops creating rows. This limit is cumulative — it counts all submissions since the webhook was created, and deleting rows does not reset it. **Enterprise plan:** Enable [auto-delete](https://www.clay.com/university/guide/auto-delete) on the table receiving the webhook to bypass this limit — auto-delete is a table-level setting accessed via the **archive icon** in the table's bottom toolbar or via the **table title** menu. You only need to enable it on the intake table, not on all downstream tables.
