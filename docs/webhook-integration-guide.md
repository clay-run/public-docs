...
2.  Search for `Webhooks` and click `Webhook`.
3.  Copy the URL/cURL.
...
1. In your Clay workbook, create a table using **Webhook** as the source (see [Creating a table with webhook](#creating-a-table-with-webhook)).
...
To push data from n8n into Clay automatically — for example, to feed a list of companies generated by an n8n workflow into a Clay enrichment table — use n8n's **HTTP Request** node:

1. In your Clay workbook, create a table with **Webhook** as the source and copy the webhook URL.
2. In your n8n workflow, add an **HTTP Request** node after the step that produces your data.
3. Set the **Method** to `POST` and paste the Clay webhook URL into the **URL** field.
4. Set **Content Type** to `JSON`.
5. In the **Body** section, map the fields you want to send to Clay (for example, `company_name` and `domain`). Each HTTP Request execution creates one new row in Clay.
6. To send multiple records (for example, 10 lookalike companies), place the HTTP Request node inside a **Loop Over Items** node — this sends one POST per item and creates one row per record in Clay.

Once rows arrive in Clay, any enrichments configured on the table — such as Find People or Work Email lookup — run automatically on each new row.
...
2. **Incorrect URL** — Confirm you copied the full webhook endpoint URL from the **Webhook** section in your table source settings (not a partial URL or the cURL command itself).
