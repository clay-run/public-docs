...
**Submission limit:** Each webhook source accepts up to 50,000 submissions. This is a cumulative lifetime count — every accepted submission increments the counter, and neither deleting nor archiving rows from the table reduces it. Once you reach this limit, Clay returns a `403 Record limit reached for webhook` error and stops accepting new webhook submissions. **Non-Enterprise plan:** Create a new webhook source — each new source starts with a fresh 50,000-submission counter. **Enterprise plan:** You can restore the existing webhook by enabling auto-delete (see below) instead of creating a new one — once passthrough mode is active, the 50,000 submission check is bypassed and the webhook immediately resumes accepting new submissions.
...
4. **Submission limit reached** — See the [Limits](#limits) section. Once a webhook source hits 50,000 submissions, Clay returns a `403 Record limit reached for webhook` error and stops creating rows. This limit is cumulative — it counts all submissions since the webhook was created, and neither deleting nor archiving rows resets it. **Enterprise plan:** Enable [auto-delete](https://www.clay.com/university/guide/auto-delete) to restore the webhook — once passthrough mode is active, the 50,000 submission check is bypassed and the webhook resumes accepting new submissions immediately, even if it was previously blocked. You do not need to create a new webhook. **Non-Enterprise plan:** Create a new webhook source — each new source starts with its own fresh 50,000-submission counter.
