...
**To split a batch payload into individual rows:** If your webhook payload contains an array of records — for example, a `people` field with multiple contacts sent in a single POST — you can expand each element into its own row in a separate destination table using [Send Table Data](https://www.clay.com/university/guide/send-table-data). After your webhook table receives the payload as a single row, add a **Send Table Data** column to that webhook table, choose **Send row for each item in a list**, and select the array field as the list source. Each element in the array becomes its own row in the destination table, where you can then run enrichments on it individually. Note that this method sends a maximum of **20 items per row** per run. If the array arrives as a stringified JSON string — common when data passes through middleware tools like Workato — switch the list input to formula mode and enter `JSON.parse(/YourField)` to convert it to a native list first.
