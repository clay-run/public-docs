...
This varies by destination, since each platform sets its own specification and Clay follows it. Google Ads takes email, phone, first name, and last name as SHA-256 hashes, while country and zip code are sent unhashed to match Google's specification. Meta Ads hashes every field Clay sends it, country and zip code included. The professional network ads destination hashes the email address and sends the remaining fields as plain text. Bing Ads hashes the email address only.
