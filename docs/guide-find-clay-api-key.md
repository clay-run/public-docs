...
If you need programmatic access to Clay from an external tool or CLI, Clay offers a Public HTTP API (`api.clay.com`) that uses a separate workspace-scoped API key — this is distinct from the personal key described above. The Public HTTP API is available on all plans. To get a workspace-scoped key, open **API and CLI** in the left sidebar, select the **API keys** tab, and click **Add API key** — or follow the setup instructions at [developers.clay.com](https://developers.clay.com) and let the Clay agent plugin create the key for you.
