...
If you need programmatic access to Clay from an external tool or CLI, Clay offers a Public HTTP API (`api.clay.com`) that uses a separate workspace-scoped API key — this is distinct from the personal key described above. The Public HTTP API is available on all plans. To get a workspace-scoped key, go to **API and CLI** in the left sidebar (under Orchestration) and open the **API keys** tab — you can create and manage workspace-scoped keys there. Alternatively, after installing the Clay agent plugin, run `clay api-keys create` to generate a key from the CLI.
