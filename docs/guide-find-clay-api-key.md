...
If you need programmatic access to Clay from an external tool or CLI, Clay offers a Public HTTP API (`api.clay.com`) that uses a separate workspace-scoped API key — this is distinct from the personal key described above. A workspace-scoped key is confined to the workspace it was issued for; requests using that key to access a different workspace will return a 403 error, even if the key owner is also a member of that other workspace. The Public HTTP API is currently in beta. To request access for your workspace, contact Clay support.

Once your workspace has access, you can manage your workspace-scoped API keys using the Clay CLI: `clay api-keys list`, `clay api-keys create`, `clay api-keys update`, and `clay api-keys delete`.
