...
If you need programmatic access to Clay from an external tool or CLI, Clay offers a Public HTTP API (`api.clay.com`) that uses a separate workspace-scoped API key — this is distinct from the personal key described above. The Public HTTP API is available on all plans. To get a workspace-scoped key, follow the setup instructions at [developers.clay.com](https://developers.clay.com) — the Clay agent plugin creates the key for you in one step. To create and manage workspace-scoped keys directly in Clay, open the **API and CLI** page from the left sidebar in your workspace and select the **API keys** tab. Workspace-scoped API keys are available to every workspace — no access request needed. You can also create a key from the CLI with `clay api-keys create`.
