...
If you need programmatic access to Clay from an external tool or CLI, Clay offers a Public HTTP API (`api.clay.com`) that uses a separate workspace-scoped API key — this is distinct from the personal key described above. The Public HTTP API is available on all plans. Workspace-scoped API keys are available to every workspace — no access request needed. You can create and manage keys from the **API and CLI → API keys** page in the left sidebar, or from the CLI with `clay api-keys create`. You can also follow the setup instructions at [developers.clay.com](https://developers.clay.com) — the Clay agent plugin creates the key for you in one step.
