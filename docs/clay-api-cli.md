...
description: Install and use the Clay Agent Plugin to run Clay programmatically from coding agents, terminals, and backend systems via the CLI and Public API.
last_synced: 2026-09-16T05:46:05.890Z
---
...
| Flex | 500 | 50k/yr |
| Launch, Growth, and other paid plans | 500 | 1M/yr |
...
Basic row reads work on any plan. Structured queries — joins, ranges, and paging past 100 rows — need API table sync, available on Trial and all paid plans. There are no current plans to support table building from the API or CLI.
