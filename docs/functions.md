...
**Note:** The function's live view displays only the most recent **1,000 rows** at a time. When the live view reaches 1,000 rows, older rows are automatically removed as new ones arrive — this is expected behavior, not a bug. Functions are passthrough tables designed to enrich data and return results to the calling table; keeping the active row count low ensures there is always space for new incoming records. All rows are fully processed and their results returned to the calling table regardless of this live-view limit.

To review rows beyond the 1,000 shown in the live view, click the **Archive** button in the function table's toolbar. The Archive section shows historically processed rows organized by timeline, and you can export the full set as a CSV. If you need to retain more active rows in the live view (for example, for easier auditing of recent runs), open the function's settings panel and update the **Number of rows to keep** field. Keep this value well below the 50,000-row hard ceiling that applies to all Clay tables to ensure enough capacity for new records to keep flowing in — though increasing this limit on high-volume functions can slow processing.
